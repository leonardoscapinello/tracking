<?php

class StaticFilesIcons
{
    private $_technology_name = "NEX-AUTOLOAD-ICONS";
    private $_path = DIRNAME . "../../dashboard/stylesheet/";
    private $_filename = "icons.min.css";
    private $_fonts_files = "fonts.css";
    private $_styles = array(
        "far" => true,
        "fas" => true,
        "nex" => true,
    );
    private $_main = array(
        "fa-500px" => "\\f26e",
        "fa-abacus" => "\\f640",
        "fa-accessible-icon" => "\\f368",
        "fa-accusoft" => "\\f369",
        "fa-acorn" => "\\f6ae",
        "fa-acquisitions-incorporated" => "\\f6af",
        "fa-ad" => "\\f641",
        "fa-address-book" => "\\f2b9",
        "fa-address-card" => "\\f2bb",
        "fa-adjust" => "\\f042",
        "fa-adn" => "\\f170",
        "fa-adobe" => "\\f778",
        "fa-adversal" => "\\f36a",
        "fa-affiliatetheme" => "\\f36b",
        "fa-air-freshener" => "\\f5d0",
        "fa-airbnb" => "\\f834",
        "fa-alarm-clock" => "\\f34e",
        "fa-alarm-exclamation" => "\\f843",
        "fa-alarm-plus" => "\\f844",
        "fa-alarm-snooze" => "\\f845",
        "fa-album" => "\\f89f",
        "fa-album-collection" => "\\f8a0",
        "fa-algolia" => "\\f36c",
        "fa-alicorn" => "\\f6b0",
        "fa-align-center" => "\\f037",
        "fa-align-justify" => "\\f039",
        "fa-align-left" => "\\f036",
        "fa-align-right" => "\\f038",
        "fa-align-slash" => "\\f846",
        "fa-alipay" => "\\f642",
        "fa-allergies" => "\\f461",
        "fa-amazon" => "\\f270",
        "fa-amazon-pay" => "\\f42c",
        "fa-ambulance" => "\\f0f9",
        "fa-american-sign-language-interpreting" => "\\f2a3",
        "fa-amilia" => "\\f36d",
        "fa-amp-guitar" => "\\f8a1",
        "fa-analytics" => "\\f643",
        "fa-anchor" => "\\f13d",
        "fa-android" => "\\f17b",
        "fa-angel" => "\\f779",
        "fa-angellist" => "\\f209",
        "fa-angle-double-down" => "\\f103",
        "fa-angle-double-left" => "\\f100",
        "fa-angle-double-right" => "\\f101",
        "fa-angle-double-up" => "\\f102",
        "fa-angle-down" => "\\f107",
        "fa-angle-left" => "\\f104",
        "fa-angle-right" => "\\f105",
        "fa-angle-up" => "\\f106",
        "fa-angry" => "\\f556",
        "fa-angrycreative" => "\\f36e",
        "fa-angular" => "\\f420",
        "fa-ankh" => "\\f644",
        "fa-app-store" => "\\f36f",
        "fa-app-store-ios" => "\\f370",
        "fa-apper" => "\\f371",
        "fa-apple" => "\\f179",
        "fa-apple-alt" => "\\f5d1",
        "fa-apple-crate" => "\\f6b1",
        "fa-apple-pay" => "\\f415",
        "fa-archive" => "\\f187",
        "fa-archway" => "\\f557",
        "fa-arrow-alt-circle-down" => "\\f358",
        "fa-arrow-alt-circle-left" => "\\f359",
        "fa-arrow-alt-circle-right" => "\\f35a",
        "fa-arrow-alt-circle-up" => "\\f35b",
        "fa-arrow-alt-down" => "\\f354",
        "fa-arrow-alt-from-bottom" => "\\f346",
        "fa-arrow-alt-from-left" => "\\f347",
        "fa-arrow-alt-from-right" => "\\f348",
        "fa-arrow-alt-from-top" => "\\f349",
        "fa-arrow-alt-left" => "\\f355",
        "fa-arrow-alt-right" => "\\f356",
        "fa-arrow-alt-square-down" => "\\f350",
        "fa-arrow-alt-square-left" => "\\f351",
        "fa-arrow-alt-square-right" => "\\f352",
        "fa-arrow-alt-square-up" => "\\f353",
        "fa-arrow-alt-to-bottom" => "\\f34a",
        "fa-arrow-alt-to-left" => "\\f34b",
        "fa-arrow-alt-to-right" => "\\f34c",
        "fa-arrow-alt-to-top" => "\\f34d",
        "fa-arrow-alt-up" => "\\f357",
        "fa-arrow-circle-down" => "\\f0ab",
        "fa-arrow-circle-left" => "\\f0a8",
        "fa-arrow-circle-right" => "\\f0a9",
        "fa-arrow-circle-up" => "\\f0aa",
        "fa-arrow-down" => "\\f063",
        "fa-arrow-from-bottom" => "\\f342",
        "fa-arrow-from-left" => "\\f343",
        "fa-arrow-from-right" => "\\f344",
        "fa-arrow-from-top" => "\\f345",
        "fa-arrow-left" => "\\f060",
        "fa-arrow-right" => "\\f061",
        "fa-arrow-square-down" => "\\f339",
        "fa-arrow-square-left" => "\\f33a",
        "fa-arrow-square-right" => "\\f33b",
        "fa-arrow-square-up" => "\\f33c",
        "fa-arrow-to-bottom" => "\\f33d",
        "fa-arrow-to-left" => "\\f33e",
        "fa-arrow-to-right" => "\\f340",
        "fa-arrow-to-top" => "\\f341",
        "fa-arrow-up" => "\\f062",
        "fa-arrows" => "\\f047",
        "fa-arrows-alt" => "\\f0b2",
        "fa-arrows-alt-h" => "\\f337",
        "fa-arrows-alt-v" => "\\f338",
        "fa-arrows-h" => "\\f07e",
        "fa-arrows-v" => "\\f07d",
        "fa-artstation" => "\\f77a",
        "fa-assistive-listening-systems" => "\\f2a2",
        "fa-asterisk" => "\\f069",
        "fa-asymmetrik" => "\\f372",
        "fa-at" => "\\f1fa",
        "fa-atlas" => "\\f558",
        "fa-atlassian" => "\\f77b",
        "fa-atom" => "\\f5d2",
        "fa-atom-alt" => "\\f5d3",
        "fa-audible" => "\\f373",
        "fa-audio-description" => "\\f29e",
        "fa-autoprefixer" => "\\f41c",
        "fa-avianex" => "\\f374",
        "fa-aviato" => "\\f421",
        "fa-award" => "\\f559",
        "fa-aws" => "\\f375",
        "fa-axe" => "\\f6b2",
        "fa-axe-battle" => "\\f6b3",
        "fa-baby" => "\\f77c",
        "fa-baby-carriage" => "\\f77d",
        "fa-backpack" => "\\f5d4",
        "fa-backspace" => "\\f55a",
        "fa-backward" => "\\f04a",
        "fa-bacon" => "\\f7e5",
        "fa-badge" => "\\f335",
        "fa-badge-check" => "\\f336",
        "fa-badge-dollar" => "\\f645",
        "fa-badge-percent" => "\\f646",
        "fa-badge-sheriff" => "\\f8a2",
        "fa-badger-honey" => "\\f6b4",
        "fa-bags-shopping" => "\\f847",
        "fa-balance-scale" => "\\f24e",
        "fa-balance-scale-left" => "\\f515",
        "fa-balance-scale-right" => "\\f516",
        "fa-ball-pile" => "\\f77e",
        "fa-ballot" => "\\f732",
        "fa-ballot-check" => "\\f733",
        "fa-ban" => "\\f05e",
        "fa-band-aid" => "\\f462",
        "fa-bandcamp" => "\\f2d5",
        "fa-banjo" => "\\f8a3",
        "fa-barcode" => "\\f02a",
        "fa-barcode-alt" => "\\f463",
        "fa-barcode-read" => "\\f464",
        "fa-barcode-scan" => "\\f465",
        "fa-bars" => "\\f0c9",
        "fa-baseball" => "\\f432",
        "fa-baseball-ball" => "\\f433",
        "fa-basketball-ball" => "\\f434",
        "fa-basketball-hoop" => "\\f435",
        "fa-bat" => "\\f6b5",
        "fa-bath" => "\\f2cd",
        "fa-battery-bolt" => "\\f376",
        "fa-battery-empty" => "\\f244",
        "fa-battery-full" => "\\f240",
        "fa-battery-half" => "\\f242",
        "fa-battery-quarter" => "\\f243",
        "fa-battery-slash" => "\\f377",
        "fa-battery-three-quarters" => "\\f241",
        "fa-battle-net" => "\\f835",
        "fa-bed" => "\\f236",
        "fa-beer" => "\\f0fc",
        "fa-behance" => "\\f1b4",
        "fa-behance-square" => "\\f1b5",
        "fa-bell" => "\\f0f3",
        "fa-bell-exclamation" => "\\f848",
        "fa-bell-plus" => "\\f849",
        "fa-bell-school" => "\\f5d5",
        "fa-bell-school-slash" => "\\f5d6",
        "fa-bell-slash" => "\\f1f6",
        "fa-bells" => "\\f77f",
        "fa-betamax" => "\\f8a4",
        "fa-bezier-curve" => "\\f55b",
        "fa-bible" => "\\f647",
        "fa-bicycle" => "\\f206",
        "fa-biking" => "\\f84a",
        "fa-biking-mountain" => "\\f84b",
        "fa-bimobject" => "\\f378",
        "fa-binoculars" => "\\f1e5",
        "fa-biohazard" => "\\f780",
        "fa-birthday-cake" => "\\f1fd",
        "fa-bitbucket" => "\\f171",
        "fa-bitcoin" => "\\f379",
        "fa-bity" => "\\f37a",
        "fa-black-tie" => "\\f27e",
        "fa-blackberry" => "\\f37b",
        "fa-blanket" => "\\f498",
        "fa-blender" => "\\f517",
        "fa-blender-phone" => "\\f6b6",
        "fa-blind" => "\\f29d",
        "fa-blog" => "\\f781",
        "fa-blogger" => "\\f37c",
        "fa-blogger-b" => "\\f37d",
        "fa-bluetooth" => "\\f293",
        "fa-bluetooth-b" => "\\f294",
        "fa-bold" => "\\f032",
        "fa-bolt" => "\\f0e7",
        "fa-bomb" => "\\f1e2",
        "fa-bone" => "\\f5d7",
        "fa-bone-break" => "\\f5d8",
        "fa-bong" => "\\f55c",
        "fa-book" => "\\f02d",
        "fa-book-alt" => "\\f5d9",
        "fa-book-dead" => "\\f6b7",
        "fa-book-heart" => "\\f499",
        "fa-book-medical" => "\\f7e6",
        "fa-book-open" => "\\f518",
        "fa-book-reader" => "\\f5da",
        "fa-book-spells" => "\\f6b8",
        "fa-book-user" => "\\f7e7",
        "fa-bookmark" => "\\f02e",
        "fa-books" => "\\f5db",
        "fa-books-medical" => "\\f7e8",
        "fa-boombox" => "\\f8a5",
        "fa-boot" => "\\f782",
        "fa-booth-curtain" => "\\f734",
        "fa-bootstrap" => "\\f836",
        "fa-border-all" => "\\f84c",
        "fa-border-bottom" => "\\f84d",
        "fa-border-center-h" => "\\f89c",
        "fa-border-center-v" => "\\f89d",
        "fa-border-inner" => "\\f84e",
        "fa-border-left" => "\\f84f",
        "fa-border-none" => "\\f850",
        "fa-border-outer" => "\\f851",
        "fa-border-right" => "\\f852",
        "fa-border-style" => "\\f853",
        "fa-border-style-alt" => "\\f854",
        "fa-border-top" => "\\f855",
        "fa-bow-arrow" => "\\f6b9",
        "fa-bowling-ball" => "\\f436",
        "fa-bowling-pins" => "\\f437",
        "fa-box" => "\\f466",
        "fa-box-alt" => "\\f49a",
        "fa-box-ballot" => "\\f735",
        "fa-box-check" => "\\f467",
        "fa-box-fragile" => "\\f49b",
        "fa-box-full" => "\\f49c",
        "fa-box-heart" => "\\f49d",
        "fa-box-open" => "\\f49e",
        "fa-box-up" => "\\f49f",
        "fa-box-usd" => "\\f4a0",
        "fa-boxes" => "\\f468",
        "fa-boxes-alt" => "\\f4a1",
        "fa-boxing-glove" => "\\f438",
        "fa-brackets" => "\\f7e9",
        "fa-brackets-curly" => "\\f7ea",
        "fa-braille" => "\\f2a1",
        "fa-brain" => "\\f5dc",
        "fa-bread-loaf" => "\\f7eb",
        "fa-bread-slice" => "\\f7ec",
        "fa-briefcase" => "\\f0b1",
        "fa-briefcase-medical" => "\\f469",
        "fa-bring-forward" => "\\f856",
        "fa-bring-front" => "\\f857",
        "fa-broadcast-tower" => "\\f519",
        "fa-broom" => "\\f51a",
        "fa-browser" => "\\f37e",
        "fa-brush" => "\\f55d",
        "fa-btc" => "\\f15a",
        "fa-buffer" => "\\f837",
        "fa-bug" => "\\f188",
        "fa-building" => "\\f1ad",
        "fa-bullhorn" => "\\f0a1",
        "fa-bullseye" => "\\f140",
        "fa-bullseye-arrow" => "\\f648",
        "fa-bullseye-pointer" => "\\f649",
        "fa-burger-soda" => "\\f858",
        "fa-burn" => "\\f46a",
        "fa-buromobelexperte" => "\\f37f",
        "fa-burrito" => "\\f7ed",
        "fa-bus" => "\\f207",
        "fa-bus-alt" => "\\f55e",
        "fa-bus-school" => "\\f5dd",
        "fa-business-time" => "\\f64a",
        "fa-buy-n-large" => "\\f8a6",
        "fa-buysellads" => "\\f20d",
        "fa-cabinet-filing" => "\\f64b",
        "fa-cactus" => "\\f8a7",
        "fa-calculator" => "\\f1ec",
        "fa-calculator-alt" => "\\f64c",
        "fa-calendar" => "\\f133",
        "fa-calendar-alt" => "\\f073",
        "fa-calendar-check" => "\\f274",
        "fa-calendar-day" => "\\f783",
        "fa-calendar-edit" => "\\f333",
        "fa-calendar-exclamation" => "\\f334",
        "fa-calendar-minus" => "\\f272",
        "fa-calendar-plus" => "\\f271",
        "fa-calendar-star" => "\\f736",
        "fa-calendar-times" => "\\f273",
        "fa-calendar-week" => "\\f784",
        "fa-camcorder" => "\\f8a8",
        "fa-camera" => "\\f030",
        "fa-camera-alt" => "\\f332",
        "fa-camera-movie" => "\\f8a9",
        "fa-camera-polaroid" => "\\f8aa",
        "fa-camera-retro" => "\\f083",
        "fa-campfire" => "\\f6ba",
        "fa-campground" => "\\f6bb",
        "fa-canadian-maple-leaf" => "\\f785",
        "fa-candle-holder" => "\\f6bc",
        "fa-candy-cane" => "\\f786",
        "fa-candy-corn" => "\\f6bd",
        "fa-cannabis" => "\\f55f",
        "fa-capsules" => "\\f46b",
        "fa-car" => "\\f1b9",
        "fa-car-alt" => "\\f5de",
        "fa-car-battery" => "\\f5df",
        "fa-car-building" => "\\f859",
        "fa-car-bump" => "\\f5e0",
        "fa-car-bus" => "\\f85a",
        "fa-car-crash" => "\\f5e1",
        "fa-car-garage" => "\\f5e2",
        "fa-car-mechanic" => "\\f5e3",
        "fa-car-side" => "\\f5e4",
        "fa-car-tilt" => "\\f5e5",
        "fa-car-wash" => "\\f5e6",
        "fa-caret-circle-down" => "\\f32d",
        "fa-caret-circle-left" => "\\f32e",
        "fa-caret-circle-right" => "\\f330",
        "fa-caret-circle-up" => "\\f331",
        "fa-caret-down" => "\\f0d7",
        "fa-caret-left" => "\\f0d9",
        "fa-caret-right" => "\\f0da",
        "fa-caret-square-down" => "\\f150",
        "fa-caret-square-left" => "\\f191",
        "fa-caret-square-right" => "\\f152",
        "fa-caret-square-up" => "\\f151",
        "fa-caret-up" => "\\f0d8",
        "fa-carrot" => "\\f787",
        "fa-cars" => "\\f85b",
        "fa-cart-arrow-down" => "\\f218",
        "fa-cart-plus" => "\\f217",
        "fa-cash-register" => "\\f788",
        "fa-cassette-tape" => "\\f8ab",
        "fa-cat" => "\\f6be",
        "fa-cauldron" => "\\f6bf",
        "fa-cc-amazon-pay" => "\\f42d",
        "fa-cc-amex" => "\\f1f3",
        "fa-cc-apple-pay" => "\\f416",
        "fa-cc-diners-club" => "\\f24c",
        "fa-cc-discover" => "\\f1f2",
        "fa-cc-jcb" => "\\f24b",
        "fa-cc-mastercard" => "\\f1f1",
        "fa-cc-paypal" => "\\f1f4",
        "fa-cc-stripe" => "\\f1f5",
        "fa-cc-visa" => "\\f1f0",
        "fa-cctv" => "\\f8ac",
        "fa-centercode" => "\\f380",
        "fa-centos" => "\\f789",
        "fa-certificate" => "\\f0a3",
        "fa-chair" => "\\f6c0",
        "fa-chair-office" => "\\f6c1",
        "fa-chalkboard" => "\\f51b",
        "fa-chalkboard-teacher" => "\\f51c",
        "fa-charging-station" => "\\f5e7",
        "fa-chart-area" => "\\f1fe",
        "fa-chart-bar" => "\\f080",
        "fa-chart-line" => "\\f201",
        "fa-chart-line-down" => "\\f64d",
        "fa-chart-network" => "\\f78a",
        "fa-chart-pie" => "\\f200",
        "fa-chart-pie-alt" => "\\f64e",
        "fa-chart-scatter" => "\\f7ee",
        "fa-check" => "\\f00c",
        "fa-check-circle" => "\\f058",
        "fa-check-double" => "\\f560",
        "fa-check-square" => "\\f14a",
        "fa-cheese" => "\\f7ef",
        "fa-cheese-swiss" => "\\f7f0",
        "fa-cheeseburger" => "\\f7f1",
        "fa-chess" => "\\f439",
        "fa-chess-bishop" => "\\f43a",
        "fa-chess-bishop-alt" => "\\f43b",
        "fa-chess-board" => "\\f43c",
        "fa-chess-clock" => "\\f43d",
        "fa-chess-clock-alt" => "\\f43e",
        "fa-chess-king" => "\\f43f",
        "fa-chess-king-alt" => "\\f440",
        "fa-chess-knight" => "\\f441",
        "fa-chess-knight-alt" => "\\f442",
        "fa-chess-pawn" => "\\f443",
        "fa-chess-pawn-alt" => "\\f444",
        "fa-chess-queen" => "\\f445",
        "fa-chess-queen-alt" => "\\f446",
        "fa-chess-rook" => "\\f447",
        "fa-chess-rook-alt" => "\\f448",
        "fa-chevron-circle-down" => "\\f13a",
        "fa-chevron-circle-left" => "\\f137",
        "fa-chevron-circle-right" => "\\f138",
        "fa-chevron-circle-up" => "\\f139",
        "fa-chevron-double-down" => "\\f322",
        "fa-chevron-double-left" => "\\f323",
        "fa-chevron-double-right" => "\\f324",
        "fa-chevron-double-up" => "\\f325",
        "fa-chevron-down" => "\\f078",
        "fa-chevron-left" => "\\f053",
        "fa-chevron-right" => "\\f054",
        "fa-chevron-square-down" => "\\f329",
        "fa-chevron-square-left" => "\\f32a",
        "fa-chevron-square-right" => "\\f32b",
        "fa-chevron-square-up" => "\\f32c",
        "fa-chevron-up" => "\\f077",
        "fa-child" => "\\f1ae",
        "fa-chimney" => "\\f78b",
        "fa-chrome" => "\\f268",
        "fa-chromecast" => "\\f838",
        "fa-church" => "\\f51d",
        "fa-circle" => "\\f111",
        "fa-circle-notch" => "\\f1ce",
        "fa-city" => "\\f64f",
        "fa-clarinet" => "\\f8ad",
        "fa-claw-marks" => "\\f6c2",
        "fa-clinic-medical" => "\\f7f2",
        "fa-clipboard" => "\\f328",
        "fa-clipboard-check" => "\\f46c",
        "fa-clipboard-list" => "\\f46d",
        "fa-clipboard-list-check" => "\\f737",
        "fa-clipboard-prescription" => "\\f5e8",
        "fa-clipboard-user" => "\\f7f3",
        "fa-clock" => "\\f017",
        "fa-clone" => "\\f24d",
        "fa-closed-captioning" => "\\f20a",
        "fa-cloud" => "\\f0c2",
        "fa-cloud-download" => "\\f0ed",
        "fa-cloud-download-alt" => "\\f381",
        "fa-cloud-drizzle" => "\\f738",
        "fa-cloud-hail" => "\\f739",
        "fa-cloud-hail-mixed" => "\\f73a",
        "fa-cloud-meatball" => "\\f73b",
        "fa-cloud-moon" => "\\f6c3",
        "fa-cloud-moon-rain" => "\\f73c",
        "fa-cloud-music" => "\\f8ae",
        "fa-cloud-rain" => "\\f73d",
        "fa-cloud-rainbow" => "\\f73e",
        "fa-cloud-showers" => "\\f73f",
        "fa-cloud-showers-heavy" => "\\f740",
        "fa-cloud-sleet" => "\\f741",
        "fa-cloud-snow" => "\\f742",
        "fa-cloud-sun" => "\\f6c4",
        "fa-cloud-sun-rain" => "\\f743",
        "fa-cloud-upload" => "\\f0ee",
        "fa-cloud-upload-alt" => "\\f382",
        "fa-clouds" => "\\f744",
        "fa-clouds-moon" => "\\f745",
        "fa-clouds-sun" => "\\f746",
        "fa-cloudscale" => "\\f383",
        "fa-cloudsmith" => "\\f384",
        "fa-cloudversify" => "\\f385",
        "fa-club" => "\\f327",
        "fa-cocktail" => "\\f561",
        "fa-code" => "\\f121",
        "fa-code-branch" => "\\f126",
        "fa-code-commit" => "\\f386",
        "fa-code-merge" => "\\f387",
        "fa-codepen" => "\\f1cb",
        "fa-codiepie" => "\\f284",
        "fa-coffee" => "\\f0f4",
        "fa-coffee-togo" => "\\f6c5",
        "fa-coffin" => "\\f6c6",
        "fa-cog" => "\\f013",
        "fa-cogs" => "\\f085",
        "fa-coin" => "\\f85c",
        "fa-coins" => "\\f51e",
        "fa-columns" => "\\f0db",
        "fa-comment" => "\\f075",
        "fa-comment-alt" => "\\f27a",
        "fa-comment-alt-check" => "\\f4a2",
        "fa-comment-alt-dollar" => "\\f650",
        "fa-comment-alt-dots" => "\\f4a3",
        "fa-comment-alt-edit" => "\\f4a4",
        "fa-comment-alt-exclamation" => "\\f4a5",
        "fa-comment-alt-lines" => "\\f4a6",
        "fa-comment-alt-medical" => "\\f7f4",
        "fa-comment-alt-minus" => "\\f4a7",
        "fa-comment-alt-music" => "\\f8af",
        "fa-comment-alt-plus" => "\\f4a8",
        "fa-comment-alt-slash" => "\\f4a9",
        "fa-comment-alt-smile" => "\\f4aa",
        "fa-comment-alt-times" => "\\f4ab",
        "fa-comment-check" => "\\f4ac",
        "fa-comment-dollar" => "\\f651",
        "fa-comment-dots" => "\\f4ad",
        "fa-comment-edit" => "\\f4ae",
        "fa-comment-exclamation" => "\\f4af",
        "fa-comment-lines" => "\\f4b0",
        "fa-comment-medical" => "\\f7f5",
        "fa-comment-minus" => "\\f4b1",
        "fa-comment-music" => "\\f8b0",
        "fa-comment-plus" => "\\f4b2",
        "fa-comment-slash" => "\\f4b3",
        "fa-comment-smile" => "\\f4b4",
        "fa-comment-times" => "\\f4b5",
        "fa-comments" => "\\f086",
        "fa-comments-alt" => "\\f4b6",
        "fa-comments-alt-dollar" => "\\f652",
        "fa-comments-dollar" => "\\f653",
        "fa-compact-disc" => "\\f51f",
        "fa-compass" => "\\f14e",
        "fa-compass-slash" => "\\f5e9",
        "fa-compress" => "\\f066",
        "fa-compress-alt" => "\\f422",
        "fa-compress-arrows-alt" => "\\f78c",
        "fa-compress-wide" => "\\f326",
        "fa-computer-classic" => "\\f8b1",
        "fa-computer-speaker" => "\\f8b2",
        "fa-concierge-bell" => "\\f562",
        "fa-confluence" => "\\f78d",
        "fa-connectdevelop" => "\\f20e",
        "fa-construction" => "\\f85d",
        "fa-container-storage" => "\\f4b7",
        "fa-contao" => "\\f26d",
        "fa-conveyor-belt" => "\\f46e",
        "fa-conveyor-belt-alt" => "\\f46f",
        "fa-cookie" => "\\f563",
        "fa-cookie-bite" => "\\f564",
        "fa-copy" => "\\f0c5",
        "fa-copyright" => "\\f1f9",
        "fa-corn" => "\\f6c7",
        "fa-cotton-bureau" => "\\f89e",
        "fa-couch" => "\\f4b8",
        "fa-cow" => "\\f6c8",
        "fa-cowbell" => "\\f8b3",
        "fa-cowbell-more" => "\\f8b4",
        "fa-cpanel" => "\\f388",
        "fa-creative-commons" => "\\f25e",
        "fa-creative-commons-by" => "\\f4e7",
        "fa-creative-commons-nc" => "\\f4e8",
        "fa-creative-commons-nc-eu" => "\\f4e9",
        "fa-creative-commons-nc-jp" => "\\f4ea",
        "fa-creative-commons-nd" => "\\f4eb",
        "fa-creative-commons-pd" => "\\f4ec",
        "fa-creative-commons-pd-alt" => "\\f4ed",
        "fa-creative-commons-remix" => "\\f4ee",
        "fa-creative-commons-sa" => "\\f4ef",
        "fa-creative-commons-sampling" => "\\f4f0",
        "fa-creative-commons-sampling-plus" => "\\f4f1",
        "fa-creative-commons-share" => "\\f4f2",
        "fa-creative-commons-zero" => "\\f4f3",
        "fa-credit-card" => "\\f09d",
        "fa-credit-card-blank" => "\\f389",
        "fa-credit-card-front" => "\\f38a",
        "fa-cricket" => "\\f449",
        "fa-critical-role" => "\\f6c9",
        "fa-croissant" => "\\f7f6",
        "fa-crop" => "\\f125",
        "fa-crop-alt" => "\\f565",
        "fa-cross" => "\\f654",
        "fa-crosshairs" => "\\f05b",
        "fa-crow" => "\\f520",
        "fa-crown" => "\\f521",
        "fa-crutch" => "\\f7f7",
        "fa-crutches" => "\\f7f8",
        "fa-css3" => "\\f13c",
        "fa-css3-alt" => "\\f38b",
        "fa-cube" => "\\f1b2",
        "fa-cubes" => "\\f1b3",
        "fa-curling" => "\\f44a",
        "fa-cut" => "\\f0c4",
        "fa-cuttlefish" => "\\f38c",
        "fa-d-and-d" => "\\f38d",
        "fa-d-and-d-beyond" => "\\f6ca",
        "fa-dagger" => "\\f6cb",
        "fa-dashcube" => "\\f210",
        "fa-database" => "\\f1c0",
        "fa-deaf" => "\\f2a4",
        "fa-debug" => "\\f7f9",
        "fa-deer" => "\\f78e",
        "fa-deer-rudolph" => "\\f78f",
        "fa-delicious" => "\\f1a5",
        "fa-democrat" => "\\f747",
        "fa-deploydog" => "\\f38e",
        "fa-deskpro" => "\\f38f",
        "fa-desktop" => "\\f108",
        "fa-desktop-alt" => "\\f390",
        "fa-dev" => "\\f6cc",
        "fa-deviantart" => "\\f1bd",
        "fa-dewpoint" => "\\f748",
        "fa-dharmachakra" => "\\f655",
        "fa-dhl" => "\\f790",
        "fa-diagnoses" => "\\f470",
        "fa-diamond" => "\\f219",
        "fa-diaspora" => "\\f791",
        "fa-dice" => "\\f522",
        "fa-dice-d10" => "\\f6cd",
        "fa-dice-d12" => "\\f6ce",
        "fa-dice-d20" => "\\f6cf",
        "fa-dice-d4" => "\\f6d0",
        "fa-dice-d6" => "\\f6d1",
        "fa-dice-d8" => "\\f6d2",
        "fa-dice-five" => "\\f523",
        "fa-dice-four" => "\\f524",
        "fa-dice-one" => "\\f525",
        "fa-dice-six" => "\\f526",
        "fa-dice-three" => "\\f527",
        "fa-dice-two" => "\\f528",
        "fa-digg" => "\\f1a6",
        "fa-digging" => "\\f85e",
        "fa-digital-ocean" => "\\f391",
        "fa-digital-tachograph" => "\\f566",
        "fa-diploma" => "\\f5ea",
        "fa-directions" => "\\f5eb",
        "fa-disc-drive" => "\\f8b5",
        "fa-discord" => "\\f392",
        "fa-discourse" => "\\f393",
        "fa-disease" => "\\f7fa",
        "fa-divide" => "\\f529",
        "fa-dizzy" => "\\f567",
        "fa-dna" => "\\f471",
        "fa-do-not-enter" => "\\f5ec",
        "fa-dochub" => "\\f394",
        "fa-docker" => "\\f395",
        "fa-dog" => "\\f6d3",
        "fa-dog-leashed" => "\\f6d4",
        "fa-dollar-sign" => "\\f155",
        "fa-dolly" => "\\f472",
        "fa-dolly-empty" => "\\f473",
        "fa-dolly-flatbed" => "\\f474",
        "fa-dolly-flatbed-alt" => "\\f475",
        "fa-dolly-flatbed-empty" => "\\f476",
        "fa-donate" => "\\f4b9",
        "fa-door-closed" => "\\f52a",
        "fa-door-open" => "\\f52b",
        "fa-dot-circle" => "\\f192",
        "fa-dove" => "\\f4ba",
        "fa-download" => "\\f019",
        "fa-draft2digital" => "\\f396",
        "fa-drafting-compass" => "\\f568",
        "fa-dragon" => "\\f6d5",
        "fa-draw-circle" => "\\f5ed",
        "fa-draw-polygon" => "\\f5ee",
        "fa-draw-square" => "\\f5ef",
        "fa-dreidel" => "\\f792",
        "fa-dribbble" => "\\f17d",
        "fa-dribbble-square" => "\\f397",
        "fa-drone" => "\\f85f",
        "fa-drone-alt" => "\\f860",
        "fa-dropbox" => "\\f16b",
        "fa-drum" => "\\f569",
        "fa-drum-steelpan" => "\\f56a",
        "fa-drumstick" => "\\f6d6",
        "fa-drumstick-bite" => "\\f6d7",
        "fa-drupal" => "\\f1a9",
        "fa-dryer" => "\\f861",
        "fa-dryer-alt" => "\\f862",
        "fa-duck" => "\\f6d8",
        "fa-dumbbell" => "\\f44b",
        "fa-dumpster" => "\\f793",
        "fa-dumpster-fire" => "\\f794",
        "fa-dungeon" => "\\f6d9",
        "fa-dyalog" => "\\f399",
        "fa-ear" => "\\f5f0",
        "fa-ear-muffs" => "\\f795",
        "fa-earlybirds" => "\\f39a",
        "fa-ebay" => "\\f4f4",
        "fa-eclipse" => "\\f749",
        "fa-eclipse-alt" => "\\f74a",
        "fa-edge" => "\\f282",
        "fa-edit" => "\\f044",
        "fa-egg" => "\\f7fb",
        "fa-egg-fried" => "\\f7fc",
        "fa-eject" => "\\f052",
        "fa-elementor" => "\\f430",
        "fa-elephant" => "\\f6da",
        "fa-ellipsis-h" => "\\f141",
        "fa-ellipsis-h-alt" => "\\f39b",
        "fa-ellipsis-v" => "\\f142",
        "fa-ellipsis-v-alt" => "\\f39c",
        "fa-ello" => "\\f5f1",
        "fa-ember" => "\\f423",
        "fa-empire" => "\\f1d1",
        "fa-empty-set" => "\\f656",
        "fa-engine-warning" => "\\f5f2",
        "fa-envelope" => "\\f0e0",
        "fa-envelope-open" => "\\f2b6",
        "fa-envelope-open-dollar" => "\\f657",
        "fa-envelope-open-text" => "\\f658",
        "fa-envelope-square" => "\\f199",
        "fa-envira" => "\\f299",
        "fa-equals" => "\\f52c",
        "fa-eraser" => "\\f12d",
        "fa-erlang" => "\\f39d",
        "fa-ethereum" => "\\f42e",
        "fa-ethernet" => "\\f796",
        "fa-etsy" => "\\f2d7",
        "fa-euro-sign" => "\\f153",
        "fa-evernote" => "\\f839",
        "fa-exchange" => "\\f0ec",
        "fa-exchange-alt" => "\\f362",
        "fa-exclamation" => "\\f12a",
        "fa-exclamation-circle" => "\\f06a",
        "fa-exclamation-square" => "\\f321",
        "fa-exclamation-triangle" => "\\f071",
        "fa-expand" => "\\f065",
        "fa-expand-alt" => "\\f424",
        "fa-expand-arrows" => "\\f31d",
        "fa-expand-arrows-alt" => "\\f31e",
        "fa-expand-wide" => "\\f320",
        "fa-expeditedssl" => "\\f23e",
        "fa-external-link" => "\\f08e",
        "fa-external-link-alt" => "\\f35d",
        "fa-external-link-square" => "\\f14c",
        "fa-external-link-square-alt" => "\\f360",
        "fa-eye" => "\\f06e",
        "fa-eye-dropper" => "\\f1fb",
        "fa-eye-evil" => "\\f6db",
        "fa-eye-slash" => "\\f070",
        "fa-facebook" => "\\f09a",
        "fa-facebook-f" => "\\f39e",
        "fa-facebook-messenger" => "\\f39f",
        "fa-facebook-square" => "\\f082",
        "fa-fan" => "\\f863",
        "fa-fantasy-flight-games" => "\\f6dc",
        "fa-farm" => "\\f864",
        "fa-fast-backward" => "\\f049",
        "fa-fast-forward" => "\\f050",
        "fa-fax" => "\\f1ac",
        "fa-feather" => "\\f52d",
        "fa-feather-alt" => "\\f56b",
        "fa-fedex" => "\\f797",
        "fa-fedora" => "\\f798",
        "fa-female" => "\\f182",
        "fa-field-hockey" => "\\f44c",
        "fa-fighter-jet" => "\\f0fb",
        "fa-figma" => "\\f799",
        "fa-file" => "\\f15b",
        "fa-file-alt" => "\\f15c",
        "fa-file-archive" => "\\f1c6",
        "fa-file-audio" => "\\f1c7",
        "fa-file-certificate" => "\\f5f3",
        "fa-file-chart-line" => "\\f659",
        "fa-file-chart-pie" => "\\f65a",
        "fa-file-check" => "\\f316",
        "fa-file-code" => "\\f1c9",
        "fa-file-contract" => "\\f56c",
        "fa-file-csv" => "\\f6dd",
        "fa-file-download" => "\\f56d",
        "fa-file-edit" => "\\f31c",
        "fa-file-excel" => "\\f1c3",
        "fa-file-exclamation" => "\\f31a",
        "fa-file-export" => "\\f56e",
        "fa-file-image" => "\\f1c5",
        "fa-file-import" => "\\f56f",
        "fa-file-invoice" => "\\f570",
        "fa-file-invoice-dollar" => "\\f571",
        "fa-file-medical" => "\\f477",
        "fa-file-medical-alt" => "\\f478",
        "fa-file-minus" => "\\f318",
        "fa-file-music" => "\\f8b6",
        "fa-file-pdf" => "\\f1c1",
        "fa-file-plus" => "\\f319",
        "fa-file-powerpoint" => "\\f1c4",
        "fa-file-prescription" => "\\f572",
        "fa-file-search" => "\\f865",
        "fa-file-signature" => "\\f573",
        "fa-file-spreadsheet" => "\\f65b",
        "fa-file-times" => "\\f317",
        "fa-file-upload" => "\\f574",
        "fa-file-user" => "\\f65c",
        "fa-file-video" => "\\f1c8",
        "fa-file-word" => "\\f1c2",
        "fa-files-medical" => "\\f7fd",
        "fa-fill" => "\\f575",
        "fa-fill-drip" => "\\f576",
        "fa-film" => "\\f008",
        "fa-film-alt" => "\\f3a0",
        "fa-film-canister" => "\\f8b7",
        "fa-filter" => "\\f0b0",
        "fa-fingerprint" => "\\f577",
        "fa-fire" => "\\f06d",
        "fa-fire-alt" => "\\f7e4",
        "fa-fire-extinguisher" => "\\f134",
        "fa-fire-smoke" => "\\f74b",
        "fa-firefox" => "\\f269",
        "fa-fireplace" => "\\f79a",
        "fa-first-aid" => "\\f479",
        "fa-first-order" => "\\f2b0",
        "fa-first-order-alt" => "\\f50a",
        "fa-firstdraft" => "\\f3a1",
        "fa-fish" => "\\f578",
        "fa-fish-cooked" => "\\f7fe",
        "fa-fist-raised" => "\\f6de",
        "fa-flag" => "\\f024",
        "fa-flag-alt" => "\\f74c",
        "fa-flag-checkered" => "\\f11e",
        "fa-flag-usa" => "\\f74d",
        "fa-flame" => "\\f6df",
        "fa-flashlight" => "\\f8b8",
        "fa-flask" => "\\f0c3",
        "fa-flask-poison" => "\\f6e0",
        "fa-flask-potion" => "\\f6e1",
        "fa-flickr" => "\\f16e",
        "fa-flipboard" => "\\f44d",
        "fa-flower" => "\\f7ff",
        "fa-flower-daffodil" => "\\f800",
        "fa-flower-tulip" => "\\f801",
        "fa-flushed" => "\\f579",
        "fa-flute" => "\\f8b9",
        "fa-flux-capacitor" => "\\f8ba",
        "fa-fly" => "\\f417",
        "fa-fog" => "\\f74e",
        "fa-folder" => "\\f07b",
        "fa-folder-minus" => "\\f65d",
        "fa-folder-open" => "\\f07c",
        "fa-folder-plus" => "\\f65e",
        "fa-folder-times" => "\\f65f",
        "fa-folder-tree" => "\\f802",
        "fa-folders" => "\\f660",
        "fa-font" => "\\f031",
        "fa-font-awesome" => "\\f2b4",
        "fa-font-awesome-alt" => "\\f35c",
        "fa-font-awesome-flag" => "\\f425",
        "fa-font-awesome-logo-full" => "\\f4e6",
        "fa-font-case" => "\\f866",
        "fa-fonticons" => "\\f280",
        "fa-fonticons-fi" => "\\f3a2",
        "fa-football-ball" => "\\f44e",
        "fa-football-helmet" => "\\f44f",
        "fa-forklift" => "\\f47a",
        "fa-fort-awesome" => "\\f286",
        "fa-fort-awesome-alt" => "\\f3a3",
        "fa-forumbee" => "\\f211",
        "fa-forward" => "\\f04e",
        "fa-foursquare" => "\\f180",
        "fa-fragile" => "\\f4bb",
        "fa-free-code-camp" => "\\f2c5",
        "fa-freebsd" => "\\f3a4",
        "fa-french-fries" => "\\f803",
        "fa-frog" => "\\f52e",
        "fa-frosty-head" => "\\f79b",
        "fa-frown" => "\\f119",
        "fa-frown-open" => "\\f57a",
        "fa-fulcrum" => "\\f50b",
        "fa-function" => "\\f661",
        "fa-funnel-dollar" => "\\f662",
        "fa-futbol" => "\\f1e3",
        "fa-galactic-republic" => "\\f50c",
        "fa-galactic-senate" => "\\f50d",
        "fa-game-board" => "\\f867",
        "fa-game-board-alt" => "\\f868",
        "fa-game-console-handheld" => "\\f8bb",
        "fa-gamepad" => "\\f11b",
        "fa-gamepad-alt" => "\\f8bc",
        "fa-gas-pump" => "\\f52f",
        "fa-gas-pump-slash" => "\\f5f4",
        "fa-gavel" => "\\f0e3",
        "fa-gem" => "\\f3a5",
        "fa-genderless" => "\\f22d",
        "fa-get-pocket" => "\\f265",
        "fa-gg" => "\\f260",
        "fa-gg-circle" => "\\f261",
        "fa-ghost" => "\\f6e2",
        "fa-gift" => "\\f06b",
        "fa-gift-card" => "\\f663",
        "fa-gifts" => "\\f79c",
        "fa-gingerbread-man" => "\\f79d",
        "fa-git" => "\\f1d3",
        "fa-git-alt" => "\\f841",
        "fa-git-square" => "\\f1d2",
        "fa-github" => "\\f09b",
        "fa-github-alt" => "\\f113",
        "fa-github-square" => "\\f092",
        "fa-gitkraken" => "\\f3a6",
        "fa-gitlab" => "\\f296",
        "fa-gitter" => "\\f426",
        "fa-glass" => "\\f804",
        "fa-glass-champagne" => "\\f79e",
        "fa-glass-cheers" => "\\f79f",
        "fa-glass-citrus" => "\\f869",
        "fa-glass-martini" => "\\f000",
        "fa-glass-martini-alt" => "\\f57b",
        "fa-glass-whiskey" => "\\f7a0",
        "fa-glass-whiskey-rocks" => "\\f7a1",
        "fa-glasses" => "\\f530",
        "fa-glasses-alt" => "\\f5f5",
        "fa-glide" => "\\f2a5",
        "fa-glide-g" => "\\f2a6",
        "fa-globe" => "\\f0ac",
        "fa-globe-africa" => "\\f57c",
        "fa-globe-americas" => "\\f57d",
        "fa-globe-asia" => "\\f57e",
        "fa-globe-europe" => "\\f7a2",
        "fa-globe-snow" => "\\f7a3",
        "fa-globe-stand" => "\\f5f6",
        "fa-gofore" => "\\f3a7",
        "fa-golf-ball" => "\\f450",
        "fa-golf-club" => "\\f451",
        "fa-goodreads" => "\\f3a8",
        "fa-goodreads-g" => "\\f3a9",
        "fa-google" => "\\f1a0",
        "fa-google-drive" => "\\f3aa",
        "fa-google-play" => "\\f3ab",
        "fa-google-plus" => "\\f2b3",
        "fa-google-plus-g" => "\\f0d5",
        "fa-google-plus-square" => "\\f0d4",
        "fa-google-wallet" => "\\f1ee",
        "fa-gopuram" => "\\f664",
        "fa-graduation-cap" => "\\f19d",
        "fa-gramophone" => "\\f8bd",
        "fa-gratipay" => "\\f184",
        "fa-grav" => "\\f2d6",
        "fa-greater-than" => "\\f531",
        "fa-greater-than-equal" => "\\f532",
        "fa-grimace" => "\\f57f",
        "fa-grin" => "\\f580",
        "fa-grin-alt" => "\\f581",
        "fa-grin-beam" => "\\f582",
        "fa-grin-beam-sweat" => "\\f583",
        "fa-grin-hearts" => "\\f584",
        "fa-grin-squint" => "\\f585",
        "fa-grin-squint-tears" => "\\f586",
        "fa-grin-stars" => "\\f587",
        "fa-grin-tears" => "\\f588",
        "fa-grin-tongue" => "\\f589",
        "fa-grin-tongue-squint" => "\\f58a",
        "fa-grin-tongue-wink" => "\\f58b",
        "fa-grin-wink" => "\\f58c",
        "fa-grip-horizontal" => "\\f58d",
        "fa-grip-lines" => "\\f7a4",
        "fa-grip-lines-vertical" => "\\f7a5",
        "fa-grip-vertical" => "\\f58e",
        "fa-gripfire" => "\\f3ac",
        "fa-grunt" => "\\f3ad",
        "fa-guitar" => "\\f7a6",
        "fa-guitar-electric" => "\\f8be",
        "fa-guitars" => "\\f8bf",
        "fa-gulp" => "\\f3ae",
        "fa-h-square" => "\\f0fd",
        "fa-h1" => "\\f313",
        "fa-h2" => "\\f314",
        "fa-h3" => "\\f315",
        "fa-h4" => "\\f86a",
        "fa-hacker-news" => "\\f1d4",
        "fa-hacker-news-square" => "\\f3af",
        "fa-hackerrank" => "\\f5f7",
        "fa-hamburger" => "\\f805",
        "fa-hammer" => "\\f6e3",
        "fa-hammer-war" => "\\f6e4",
        "fa-hamsa" => "\\f665",
        "fa-hand-heart" => "\\f4bc",
        "fa-hand-holding" => "\\f4bd",
        "fa-hand-holding-box" => "\\f47b",
        "fa-hand-holding-heart" => "\\f4be",
        "fa-hand-holding-magic" => "\\f6e5",
        "fa-hand-holding-seedling" => "\\f4bf",
        "fa-hand-holding-usd" => "\\f4c0",
        "fa-hand-holding-water" => "\\f4c1",
        "fa-hand-lizard" => "\\f258",
        "fa-hand-middle-finger" => "\\f806",
        "fa-hand-paper" => "\\f256",
        "fa-hand-peace" => "\\f25b",
        "fa-hand-point-down" => "\\f0a7",
        "fa-hand-point-left" => "\\f0a5",
        "fa-hand-point-right" => "\\f0a4",
        "fa-hand-point-up" => "\\f0a6",
        "fa-hand-pointer" => "\\f25a",
        "fa-hand-receiving" => "\\f47c",
        "fa-hand-rock" => "\\f255",
        "fa-hand-scissors" => "\\f257",
        "fa-hand-spock" => "\\f259",
        "fa-hands" => "\\f4c2",
        "fa-hands-heart" => "\\f4c3",
        "fa-hands-helping" => "\\f4c4",
        "fa-hands-usd" => "\\f4c5",
        "fa-handshake" => "\\f2b5",
        "fa-handshake-alt" => "\\f4c6",
        "fa-hanukiah" => "\\f6e6",
        "fa-hard-hat" => "\\f807",
        "fa-hashtag" => "\\f292",
        "fa-hat-chef" => "\\f86b",
        "fa-hat-cowboy" => "\\f8c0",
        "fa-hat-cowboy-side" => "\\f8c1",
        "fa-hat-santa" => "\\f7a7",
        "fa-hat-winter" => "\\f7a8",
        "fa-hat-witch" => "\\f6e7",
        "fa-hat-wizard" => "\\f6e8",
        "fa-haykal" => "\\f666",
        "fa-hdd" => "\\f0a0",
        "fa-head-side" => "\\f6e9",
        "fa-head-side-brain" => "\\f808",
        "fa-head-side-headphones" => "\\f8c2",
        "fa-head-side-medical" => "\\f809",
        "fa-head-vr" => "\\f6ea",
        "fa-heading" => "\\f1dc",
        "fa-headphones" => "\\f025",
        "fa-headphones-alt" => "\\f58f",
        "fa-headset" => "\\f590",
        "fa-heart" => "\\f004",
        "fa-heart-broken" => "\\f7a9",
        "fa-heart-circle" => "\\f4c7",
        "fa-heart-rate" => "\\f5f8",
        "fa-heart-square" => "\\f4c8",
        "fa-heartbeat" => "\\f21e",
        "fa-helicopter" => "\\f533",
        "fa-helmet-battle" => "\\f6eb",
        "fa-hexagon" => "\\f312",
        "fa-highlighter" => "\\f591",
        "fa-hiking" => "\\f6ec",
        "fa-hippo" => "\\f6ed",
        "fa-hips" => "\\f452",
        "fa-hire-a-helper" => "\\f3b0",
        "fa-history" => "\\f1da",
        "fa-hockey-mask" => "\\f6ee",
        "fa-hockey-puck" => "\\f453",
        "fa-hockey-sticks" => "\\f454",
        "fa-holly-berry" => "\\f7aa",
        "fa-home" => "\\f015",
        "fa-home-alt" => "\\f80a",
        "fa-home-heart" => "\\f4c9",
        "fa-home-lg" => "\\f80b",
        "fa-home-lg-alt" => "\\f80c",
        "fa-hood-cloak" => "\\f6ef",
        "fa-hooli" => "\\f427",
        "fa-horizontal-rule" => "\\f86c",
        "fa-hornbill" => "\\f592",
        "fa-horse" => "\\f6f0",
        "fa-horse-head" => "\\f7ab",
        "fa-horse-saddle" => "\\f8c3",
        "fa-hospital" => "\\f0f8",
        "fa-hospital-alt" => "\\f47d",
        "fa-hospital-symbol" => "\\f47e",
        "fa-hospital-user" => "\\f80d",
        "fa-hospitals" => "\\f80e",
        "fa-hot-tub" => "\\f593",
        "fa-hotdog" => "\\f80f",
        "fa-hotel" => "\\f594",
        "fa-hotjar" => "\\f3b1",
        "fa-hourglass" => "\\f254",
        "fa-hourglass-end" => "\\f253",
        "fa-hourglass-half" => "\\f252",
        "fa-hourglass-start" => "\\f251",
        "fa-house-damage" => "\\f6f1",
        "fa-house-flood" => "\\f74f",
        "fa-houzz" => "\\f27c",
        "fa-hryvnia" => "\\f6f2",
        "fa-html5" => "\\f13b",
        "fa-hubspot" => "\\f3b2",
        "fa-humidity" => "\\f750",
        "fa-hurricane" => "\\f751",
        "fa-i-cursor" => "\\f246",
        "fa-ice-cream" => "\\f810",
        "fa-ice-skate" => "\\f7ac",
        "fa-icicles" => "\\f7ad",
        "fa-icons" => "\\f86d",
        "fa-icons-alt" => "\\f86e",
        "fa-id-badge" => "\\f2c1",
        "fa-id-card" => "\\f2c2",
        "fa-id-card-alt" => "\\f47f",
        "fa-igloo" => "\\f7ae",
        "fa-image" => "\\f03e",
        "fa-image-polaroid" => "\\f8c4",
        "fa-images" => "\\f302",
        "fa-imdb" => "\\f2d8",
        "fa-inbox" => "\\f01c",
        "fa-inbox-in" => "\\f310",
        "fa-inbox-out" => "\\f311",
        "fa-indent" => "\\f03c",
        "fa-industry" => "\\f275",
        "fa-industry-alt" => "\\f3b3",
        "fa-infinity" => "\\f534",
        "fa-info" => "\\f129",
        "fa-info-circle" => "\\f05a",
        "fa-info-square" => "\\f30f",
        "fa-inhaler" => "\\f5f9",
        "fa-instagram" => "\\f16d",
        "fa-integral" => "\\f667",
        "fa-intercom" => "\\f7af",
        "fa-internet-explorer" => "\\f26b",
        "fa-intersection" => "\\f668",
        "fa-inventory" => "\\f480",
        "fa-invision" => "\\f7b0",
        "fa-ioxhost" => "\\f208",
        "fa-island-tropical" => "\\f811",
        "fa-italic" => "\\f033",
        "fa-itch-io" => "\\f83a",
        "fa-itunes" => "\\f3b4",
        "fa-itunes-note" => "\\f3b5",
        "fa-jack-o-lantern" => "\\f30e",
        "fa-java" => "\\f4e4",
        "fa-jedi" => "\\f669",
        "fa-jedi-order" => "\\f50e",
        "fa-jenkins" => "\\f3b6",
        "fa-jira" => "\\f7b1",
        "fa-joget" => "\\f3b7",
        "fa-joint" => "\\f595",
        "fa-joomla" => "\\f1aa",
        "fa-journal-whills" => "\\f66a",
        "fa-joystick" => "\\f8c5",
        "fa-js" => "\\f3b8",
        "fa-js-square" => "\\f3b9",
        "fa-jsfiddle" => "\\f1cc",
        "fa-jug" => "\\f8c6",
        "fa-kaaba" => "\\f66b",
        "fa-kaggle" => "\\f5fa",
        "fa-kazoo" => "\\f8c7",
        "fa-kerning" => "\\f86f",
        "fa-key" => "\\f084",
        "fa-key-skeleton" => "\\f6f3",
        "fa-keybase" => "\\f4f5",
        "fa-keyboard" => "\\f11c",
        "fa-keycdn" => "\\f3ba",
        "fa-keynote" => "\\f66c",
        "fa-khanda" => "\\f66d",
        "fa-kickstarter" => "\\f3bb",
        "fa-kickstarter-k" => "\\f3bc",
        "fa-kidneys" => "\\f5fb",
        "fa-kiss" => "\\f596",
        "fa-kiss-beam" => "\\f597",
        "fa-kiss-wink-heart" => "\\f598",
        "fa-kite" => "\\f6f4",
        "fa-kiwi-bird" => "\\f535",
        "fa-knife-kitchen" => "\\f6f5",
        "fa-korvue" => "\\f42f",
        "fa-lambda" => "\\f66e",
        "fa-lamp" => "\\f4ca",
        "fa-landmark" => "\\f66f",
        "fa-landmark-alt" => "\\f752",
        "fa-language" => "\\f1ab",
        "fa-laptop" => "\\f109",
        "fa-laptop-code" => "\\f5fc",
        "fa-laptop-medical" => "\\f812",
        "fa-laravel" => "\\f3bd",
        "fa-lasso" => "\\f8c8",
        "fa-lastfm" => "\\f202",
        "fa-lastfm-square" => "\\f203",
        "fa-laugh" => "\\f599",
        "fa-laugh-beam" => "\\f59a",
        "fa-laugh-squint" => "\\f59b",
        "fa-laugh-wink" => "\\f59c",
        "fa-layer-group" => "\\f5fd",
        "fa-layer-minus" => "\\f5fe",
        "fa-layer-plus" => "\\f5ff",
        "fa-leaf" => "\\f06c",
        "fa-leaf-heart" => "\\f4cb",
        "fa-leaf-maple" => "\\f6f6",
        "fa-leaf-oak" => "\\f6f7",
        "fa-leanpub" => "\\f212",
        "fa-lemon" => "\\f094",
        "fa-less" => "\\f41d",
        "fa-less-than" => "\\f536",
        "fa-less-than-equal" => "\\f537",
        "fa-level-down" => "\\f149",
        "fa-level-down-alt" => "\\f3be",
        "fa-level-up" => "\\f148",
        "fa-level-up-alt" => "\\f3bf",
        "fa-life-ring" => "\\f1cd",
        "fa-lightbulb" => "\\f0eb",
        "fa-lightbulb-dollar" => "\\f670",
        "fa-lightbulb-exclamation" => "\\f671",
        "fa-lightbulb-on" => "\\f672",
        "fa-lightbulb-slash" => "\\f673",
        "fa-lights-holiday" => "\\f7b2",
        "fa-line" => "\\f3c0",
        "fa-line-columns" => "\\f870",
        "fa-line-height" => "\\f871",
        "fa-link" => "\\f0c1",
        "fa-linkedin" => "\\f08c",
        "fa-linkedin-in" => "\\f0e1",
        "fa-linode" => "\\f2b8",
        "fa-linux" => "\\f17c",
        "fa-lips" => "\\f600",
        "fa-lira-sign" => "\\f195",
        "fa-list" => "\\f03a",
        "fa-list-alt" => "\\f022",
        "fa-list-music" => "\\f8c9",
        "fa-list-ol" => "\\f0cb",
        "fa-list-ul" => "\\f0ca",
        "fa-location" => "\\f601",
        "fa-location-arrow" => "\\f124",
        "fa-location-circle" => "\\f602",
        "fa-location-slash" => "\\f603",
        "fa-lock" => "\\f023",
        "fa-lock-alt" => "\\f30d",
        "fa-lock-open" => "\\f3c1",
        "fa-lock-open-alt" => "\\f3c2",
        "fa-long-arrow-alt-down" => "\\f309",
        "fa-long-arrow-alt-left" => "\\f30a",
        "fa-long-arrow-alt-right" => "\\f30b",
        "fa-long-arrow-alt-up" => "\\f30c",
        "fa-long-arrow-down" => "\\f175",
        "fa-long-arrow-left" => "\\f177",
        "fa-long-arrow-right" => "\\f178",
        "fa-long-arrow-up" => "\\f176",
        "fa-loveseat" => "\\f4cc",
        "fa-low-vision" => "\\f2a8",
        "fa-luchador" => "\\f455",
        "fa-luggage-cart" => "\\f59d",
        "fa-lungs" => "\\f604",
        "fa-lyft" => "\\f3c3",
        "fa-mace" => "\\f6f8",
        "fa-magento" => "\\f3c4",
        "fa-magic" => "\\f0d0",
        "fa-magnet" => "\\f076",
        "fa-mail-bulk" => "\\f674",
        "fa-mailbox" => "\\f813",
        "fa-mailchimp" => "\\f59e",
        "fa-male" => "\\f183",
        "fa-mandalorian" => "\\f50f",
        "fa-mandolin" => "\\f6f9",
        "fa-map" => "\\f279",
        "fa-map-marked" => "\\f59f",
        "fa-map-marked-alt" => "\\f5a0",
        "fa-map-marker" => "\\f041",
        "fa-map-marker-alt" => "\\f3c5",
        "fa-map-marker-alt-slash" => "\\f605",
        "fa-map-marker-check" => "\\f606",
        "fa-map-marker-edit" => "\\f607",
        "fa-map-marker-exclamation" => "\\f608",
        "fa-map-marker-minus" => "\\f609",
        "fa-map-marker-plus" => "\\f60a",
        "fa-map-marker-question" => "\\f60b",
        "fa-map-marker-slash" => "\\f60c",
        "fa-map-marker-smile" => "\\f60d",
        "fa-map-marker-times" => "\\f60e",
        "fa-map-pin" => "\\f276",
        "fa-map-signs" => "\\f277",
        "fa-markdown" => "\\f60f",
        "fa-marker" => "\\f5a1",
        "fa-mars" => "\\f222",
        "fa-mars-double" => "\\f227",
        "fa-mars-stroke" => "\\f229",
        "fa-mars-stroke-h" => "\\f22b",
        "fa-mars-stroke-v" => "\\f22a",
        "fa-mask" => "\\f6fa",
        "fa-mastodon" => "\\f4f6",
        "fa-maxcdn" => "\\f136",
        "fa-mdb" => "\\f8ca",
        "fa-meat" => "\\f814",
        "fa-medal" => "\\f5a2",
        "fa-medapps" => "\\f3c6",
        "fa-medium" => "\\f23a",
        "fa-medium-m" => "\\f3c7",
        "fa-medkit" => "\\f0fa",
        "fa-medrt" => "\\f3c8",
        "fa-meetup" => "\\f2e0",
        "fa-megaphone" => "\\f675",
        "fa-megaport" => "\\f5a3",
        "fa-meh" => "\\f11a",
        "fa-meh-blank" => "\\f5a4",
        "fa-meh-rolling-eyes" => "\\f5a5",
        "fa-memory" => "\\f538",
        "fa-mendeley" => "\\f7b3",
        "fa-menorah" => "\\f676",
        "fa-mercury" => "\\f223",
        "fa-meteor" => "\\f753",
        "fa-microchip" => "\\f2db",
        "fa-microphone" => "\\f130",
        "fa-microphone-alt" => "\\f3c9",
        "fa-microphone-alt-slash" => "\\f539",
        "fa-microphone-slash" => "\\f131",
        "fa-microphone-stand" => "\\f8cb",
        "fa-microscope" => "\\f610",
        "fa-microsoft" => "\\f3ca",
        "fa-mind-share" => "\\f677",
        "fa-minus" => "\\f068",
        "fa-minus-circle" => "\\f056",
        "fa-minus-hexagon" => "\\f307",
        "fa-minus-octagon" => "\\f308",
        "fa-minus-square" => "\\f146",
        "fa-mistletoe" => "\\f7b4",
        "fa-mitten" => "\\f7b5",
        "fa-mix" => "\\f3cb",
        "fa-mixcloud" => "\\f289",
        "fa-mizuni" => "\\f3cc",
        "fa-mobile" => "\\f10b",
        "fa-mobile-alt" => "\\f3cd",
        "fa-mobile-android" => "\\f3ce",
        "fa-mobile-android-alt" => "\\f3cf",
        "fa-modx" => "\\f285",
        "fa-monero" => "\\f3d0",
        "fa-money-bill" => "\\f0d6",
        "fa-money-bill-alt" => "\\f3d1",
        "fa-money-bill-wave" => "\\f53a",
        "fa-money-bill-wave-alt" => "\\f53b",
        "fa-money-check" => "\\f53c",
        "fa-money-check-alt" => "\\f53d",
        "fa-money-check-edit" => "\\f872",
        "fa-money-check-edit-alt" => "\\f873",
        "fa-monitor-heart-rate" => "\\f611",
        "fa-monkey" => "\\f6fb",
        "fa-monument" => "\\f5a6",
        "fa-moon" => "\\f186",
        "fa-moon-cloud" => "\\f754",
        "fa-moon-stars" => "\\f755",
        "fa-mortar-pestle" => "\\f5a7",
        "fa-mosque" => "\\f678",
        "fa-motorcycle" => "\\f21c",
        "fa-mountain" => "\\f6fc",
        "fa-mountains" => "\\f6fd",
        "fa-mouse" => "\\f8cc",
        "fa-mouse-alt" => "\\f8cd",
        "fa-mouse-pointer" => "\\f245",
        "fa-mp3-player" => "\\f8ce",
        "fa-mug" => "\\f874",
        "fa-mug-hot" => "\\f7b6",
        "fa-mug-marshmallows" => "\\f7b7",
        "fa-mug-tea" => "\\f875",
        "fa-music" => "\\f001",
        "fa-music-alt" => "\\f8cf",
        "fa-music-alt-slash" => "\\f8d0",
        "fa-music-slash" => "\\f8d1",
        "fa-napster" => "\\f3d2",
        "fa-narwhal" => "\\f6fe",
        "fa-neos" => "\\f612",
        "fa-network-wired" => "\\f6ff",
        "fa-neuter" => "\\f22c",
        "fa-newspaper" => "\\f1ea",
        "fa-nimblr" => "\\f5a8",
        "fa-node" => "\\f419",
        "fa-node-js" => "\\f3d3",
        "fa-not-equal" => "\\f53e",
        "fa-notes-medical" => "\\f481",
        "fa-npm" => "\\f3d4",
        "fa-ns8" => "\\f3d5",
        "fa-nutritionix" => "\\f3d6",
        "fa-object-group" => "\\f247",
        "fa-object-ungroup" => "\\f248",
        "fa-octagon" => "\\f306",
        "fa-odnoklassniki" => "\\f263",
        "fa-odnoklassniki-square" => "\\f264",
        "fa-oil-can" => "\\f613",
        "fa-oil-temp" => "\\f614",
        "fa-old-republic" => "\\f510",
        "fa-om" => "\\f679",
        "fa-omega" => "\\f67a",
        "fa-opencart" => "\\f23d",
        "fa-openid" => "\\f19b",
        "fa-opera" => "\\f26a",
        "fa-optin-monster" => "\\f23c",
        "fa-orcid" => "\\f8d2",
        "fa-ornament" => "\\f7b8",
        "fa-osi" => "\\f41a",
        "fa-otter" => "\\f700",
        "fa-outdent" => "\\f03b",
        "fa-overline" => "\\f876",
        "fa-page-break" => "\\f877",
        "fa-page4" => "\\f3d7",
        "fa-pagelines" => "\\f18c",
        "fa-pager" => "\\f815",
        "fa-paint-brush" => "\\f1fc",
        "fa-paint-brush-alt" => "\\f5a9",
        "fa-paint-roller" => "\\f5aa",
        "fa-palette" => "\\f53f",
        "fa-palfed" => "\\f3d8",
        "fa-pallet" => "\\f482",
        "fa-pallet-alt" => "\\f483",
        "fa-paper-plane" => "\\f1d8",
        "fa-paperclip" => "\\f0c6",
        "fa-parachute-box" => "\\f4cd",
        "fa-paragraph" => "\\f1dd",
        "fa-paragraph-rtl" => "\\f878",
        "fa-parking" => "\\f540",
        "fa-parking-circle" => "\\f615",
        "fa-parking-circle-slash" => "\\f616",
        "fa-parking-slash" => "\\f617",
        "fa-passport" => "\\f5ab",
        "fa-pastafarianism" => "\\f67b",
        "fa-paste" => "\\f0ea",
        "fa-patreon" => "\\f3d9",
        "fa-pause" => "\\f04c",
        "fa-pause-circle" => "\\f28b",
        "fa-paw" => "\\f1b0",
        "fa-paw-alt" => "\\f701",
        "fa-paw-claws" => "\\f702",
        "fa-paypal" => "\\f1ed",
        "fa-peace" => "\\f67c",
        "fa-pegasus" => "\\f703",
        "fa-pen" => "\\f304",
        "fa-pen-alt" => "\\f305",
        "fa-pen-fancy" => "\\f5ac",
        "fa-pen-nib" => "\\f5ad",
        "fa-pen-square" => "\\f14b",
        "fa-pencil" => "\\f040",
        "fa-pencil-alt" => "\\f303",
        "fa-pencil-paintbrush" => "\\f618",
        "fa-pencil-ruler" => "\\f5ae",
        "fa-pennant" => "\\f456",
        "fa-penny-arcade" => "\\f704",
        "fa-people-carry" => "\\f4ce",
        "fa-pepper-hot" => "\\f816",
        "fa-percent" => "\\f295",
        "fa-percentage" => "\\f541",
        "fa-periscope" => "\\f3da",
        "fa-person-booth" => "\\f756",
        "fa-person-carry" => "\\f4cf",
        "fa-person-dolly" => "\\f4d0",
        "fa-person-dolly-empty" => "\\f4d1",
        "fa-person-sign" => "\\f757",
        "fa-phabricator" => "\\f3db",
        "fa-phoenix-framework" => "\\f3dc",
        "fa-phoenix-squadron" => "\\f511",
        "fa-phone" => "\\f095",
        "fa-phone-alt" => "\\f879",
        "fa-phone-laptop" => "\\f87a",
        "fa-phone-office" => "\\f67d",
        "fa-phone-plus" => "\\f4d2",
        "fa-phone-rotary" => "\\f8d3",
        "fa-phone-slash" => "\\f3dd",
        "fa-phone-square" => "\\f098",
        "fa-phone-square-alt" => "\\f87b",
        "fa-phone-volume" => "\\f2a0",
        "fa-photo-video" => "\\f87c",
        "fa-php" => "\\f457",
        "fa-pi" => "\\f67e",
        "fa-piano" => "\\f8d4",
        "fa-piano-keyboard" => "\\f8d5",
        "fa-pie" => "\\f705",
        "fa-pied-piper" => "\\f2ae",
        "fa-pied-piper-alt" => "\\f1a8",
        "fa-pied-piper-hat" => "\\f4e5",
        "fa-pied-piper-pp" => "\\f1a7",
        "fa-pig" => "\\f706",
        "fa-piggy-bank" => "\\f4d3",
        "fa-pills" => "\\f484",
        "fa-pinterest" => "\\f0d2",
        "fa-pinterest-p" => "\\f231",
        "fa-pinterest-square" => "\\f0d3",
        "fa-pizza" => "\\f817",
        "fa-pizza-slice" => "\\f818",
        "fa-place-of-worship" => "\\f67f",
        "fa-plane" => "\\f072",
        "fa-plane-alt" => "\\f3de",
        "fa-plane-arrival" => "\\f5af",
        "fa-plane-departure" => "\\f5b0",
        "fa-play" => "\\f04b",
        "fa-play-circle" => "\\f144",
        "fa-playstation" => "\\f3df",
        "fa-plug" => "\\f1e6",
        "fa-plus" => "\\f067",
        "fa-plus-circle" => "\\f055",
        "fa-plus-hexagon" => "\\f300",
        "fa-plus-octagon" => "\\f301",
        "fa-plus-square" => "\\f0fe",
        "fa-podcast" => "\\f2ce",
        "fa-podium" => "\\f680",
        "fa-podium-star" => "\\f758",
        "fa-poll" => "\\f681",
        "fa-poll-h" => "\\f682",
        "fa-poll-people" => "\\f759",
        "fa-poo" => "\\f2fe",
        "fa-poo-storm" => "\\f75a",
        "fa-poop" => "\\f619",
        "fa-popcorn" => "\\f819",
        "fa-portrait" => "\\f3e0",
        "fa-pound-sign" => "\\f154",
        "fa-power-off" => "\\f011",
        "fa-pray" => "\\f683",
        "fa-praying-hands" => "\\f684",
        "fa-prescription" => "\\f5b1",
        "fa-prescription-bottle" => "\\f485",
        "fa-prescription-bottle-alt" => "\\f486",
        "fa-presentation" => "\\f685",
        "fa-print" => "\\f02f",
        "fa-print-search" => "\\f81a",
        "fa-print-slash" => "\\f686",
        "fa-procedures" => "\\f487",
        "fa-product-hunt" => "\\f288",
        "fa-_project-diagram" => "\\f542",
        "fa-projector" => "\\f8d6",
        "fa-pumpkin" => "\\f707",
        "fa-pushed" => "\\f3e1",
        "fa-puzzle-piece" => "\\f12e",
        "fa-python" => "\\f3e2",
        "fa-qq" => "\\f1d6",
        "fa-qrcode" => "\\f029",
        "fa-question" => "\\f128",
        "fa-question-circle" => "\\f059",
        "fa-question-square" => "\\f2fd",
        "fa-quidditch" => "\\f458",
        "fa-quinscape" => "\\f459",
        "fa-quora" => "\\f2c4",
        "fa-quote-left" => "\\f10d",
        "fa-quote-right" => "\\f10e",
        "fa-quran" => "\\f687",
        "fa-r-_project" => "\\f4f7",
        "fa-rabbit" => "\\f708",
        "fa-rabbit-fast" => "\\f709",
        "fa-racquet" => "\\f45a",
        "fa-radiation" => "\\f7b9",
        "fa-radiation-alt" => "\\f7ba",
        "fa-radio" => "\\f8d7",
        "fa-radio-alt" => "\\f8d8",
        "fa-rainbow" => "\\f75b",
        "fa-raindrops" => "\\f75c",
        "fa-ram" => "\\f70a",
        "fa-ramp-loading" => "\\f4d4",
        "fa-random" => "\\f074",
        "fa-raspberry-pi" => "\\f7bb",
        "fa-ravelry" => "\\f2d9",
        "fa-react" => "\\f41b",
        "fa-reacteurope" => "\\f75d",
        "fa-readme" => "\\f4d5",
        "fa-rebel" => "\\f1d0",
        "fa-receipt" => "\\f543",
        "fa-record-vinyl" => "\\f8d9",
        "fa-rectangle-landscape" => "\\f2fa",
        "fa-rectangle-portrait" => "\\f2fb",
        "fa-rectangle-wide" => "\\f2fc",
        "fa-recycle" => "\\f1b8",
        "fa-red-river" => "\\f3e3",
        "fa-reddit" => "\\f1a1",
        "fa-reddit-alien" => "\\f281",
        "fa-reddit-square" => "\\f1a2",
        "fa-redhat" => "\\f7bc",
        "fa-redo" => "\\f01e",
        "fa-redo-alt" => "\\f2f9",
        "fa-registered" => "\\f25d",
        "fa-remove-format" => "\\f87d",
        "fa-renren" => "\\f18b",
        "fa-repeat" => "\\f363",
        "fa-repeat-1" => "\\f365",
        "fa-repeat-1-alt" => "\\f366",
        "fa-repeat-alt" => "\\f364",
        "fa-reply" => "\\f3e5",
        "fa-reply-all" => "\\f122",
        "fa-replyd" => "\\f3e6",
        "fa-republican" => "\\f75e",
        "fa-researchgate" => "\\f4f8",
        "fa-resolving" => "\\f3e7",
        "fa-restroom" => "\\f7bd",
        "fa-retweet" => "\\f079",
        "fa-retweet-alt" => "\\f361",
        "fa-rev" => "\\f5b2",
        "fa-ribbon" => "\\f4d6",
        "fa-ring" => "\\f70b",
        "fa-rings-wedding" => "\\f81b",
        "fa-road" => "\\f018",
        "fa-robot" => "\\f544",
        "fa-rocket" => "\\f135",
        "fa-rocketchat" => "\\f3e8",
        "fa-rockrms" => "\\f3e9",
        "fa-route" => "\\f4d7",
        "fa-route-highway" => "\\f61a",
        "fa-route-interstate" => "\\f61b",
        "fa-router" => "\\f8da",
        "fa-rss" => "\\f09e",
        "fa-rss-square" => "\\f143",
        "fa-ruble-sign" => "\\f158",
        "fa-ruler" => "\\f545",
        "fa-ruler-combined" => "\\f546",
        "fa-ruler-horizontal" => "\\f547",
        "fa-ruler-triangle" => "\\f61c",
        "fa-ruler-vertical" => "\\f548",
        "fa-running" => "\\f70c",
        "fa-rupee-sign" => "\\f156",
        "fa-rv" => "\\f7be",
        "fa-sack" => "\\f81c",
        "fa-sack-dollar" => "\\f81d",
        "fa-sad-cry" => "\\f5b3",
        "fa-sad-tear" => "\\f5b4",
        "fa-safari" => "\\f267",
        "fa-salad" => "\\f81e",
        "fa-salesforce" => "\\f83b",
        "fa-sandwich" => "\\f81f",
        "fa-sass" => "\\f41e",
        "fa-satellite" => "\\f7bf",
        "fa-satellite-dish" => "\\f7c0",
        "fa-sausage" => "\\f820",
        "fa-save" => "\\f0c7",
        "fa-sax-hot" => "\\f8db",
        "fa-saxophone" => "\\f8dc",
        "fa-scalpel" => "\\f61d",
        "fa-scalpel-path" => "\\f61e",
        "fa-scanner" => "\\f488",
        "fa-scanner-image" => "\\f8f3",
        "fa-scanner-keyboard" => "\\f489",
        "fa-scanner-touchscreen" => "\\f48a",
        "fa-scarecrow" => "\\f70d",
        "fa-scarf" => "\\f7c1",
        "fa-schlix" => "\\f3ea",
        "fa-school" => "\\f549",
        "fa-screwdriver" => "\\f54a",
        "fa-scribd" => "\\f28a",
        "fa-scroll" => "\\f70e",
        "fa-scroll-old" => "\\f70f",
        "fa-scrubber" => "\\f2f8",
        "fa-scythe" => "\\f710",
        "fa-sd-card" => "\\f7c2",
        "fa-search" => "\\f002",
        "fa-search-dollar" => "\\f688",
        "fa-search-location" => "\\f689",
        "fa-search-minus" => "\\f010",
        "fa-search-plus" => "\\f00e",
        "fa-searchengin" => "\\f3eb",
        "fa-seedling" => "\\f4d8",
        "fa-sellcast" => "\\f2da",
        "fa-sellsy" => "\\f213",
        "fa-send-back" => "\\f87e",
        "fa-send-backward" => "\\f87f",
        "fa-server" => "\\f233",
        "fa-servicestack" => "\\f3ec",
        "fa-shapes" => "\\f61f",
        "fa-share" => "\\f064",
        "fa-share-all" => "\\f367",
        "fa-share-alt" => "\\f1e0",
        "fa-share-alt-square" => "\\f1e1",
        "fa-share-square" => "\\f14d",
        "fa-sheep" => "\\f711",
        "fa-shekel-sign" => "\\f20b",
        "fa-shield" => "\\f132",
        "fa-shield-alt" => "\\f3ed",
        "fa-shield-check" => "\\f2f7",
        "fa-shield-cross" => "\\f712",
        "fa-ship" => "\\f21a",
        "fa-shipping-fast" => "\\f48b",
        "fa-shipping-timed" => "\\f48c",
        "fa-shirtsinbulk" => "\\f214",
        "fa-shish-kebab" => "\\f821",
        "fa-shoe-prints" => "\\f54b",
        "fa-shopping-bag" => "\\f290",
        "fa-shopping-basket" => "\\f291",
        "fa-shopping-cart" => "\\f07a",
        "fa-shopware" => "\\f5b5",
        "fa-shovel" => "\\f713",
        "fa-shovel-snow" => "\\f7c3",
        "fa-shower" => "\\f2cc",
        "fa-shredder" => "\\f68a",
        "fa-shuttle-van" => "\\f5b6",
        "fa-shuttlecock" => "\\f45b",
        "fa-sickle" => "\\f822",
        "fa-sigma" => "\\f68b",
        "fa-sign" => "\\f4d9",
        "fa-sign-in" => "\\f090",
        "fa-sign-in-alt" => "\\f2f6",
        "fa-sign-language" => "\\f2a7",
        "fa-sign-out" => "\\f08b",
        "fa-sign-out-alt" => "\\f2f5",
        "fa-signal" => "\\f012",
        "fa-signal-1" => "\\f68c",
        "fa-signal-2" => "\\f68d",
        "fa-signal-3" => "\\f68e",
        "fa-signal-4" => "\\f68f",
        "fa-signal-alt" => "\\f690",
        "fa-signal-alt-1" => "\\f691",
        "fa-signal-alt-2" => "\\f692",
        "fa-signal-alt-3" => "\\f693",
        "fa-signal-alt-slash" => "\\f694",
        "fa-signal-slash" => "\\f695",
        "fa-signal-stream" => "\\f8dd",
        "fa-signature" => "\\f5b7",
        "fa-sim-card" => "\\f7c4",
        "fa-simplybuilt" => "\\f215",
        "fa-sistrix" => "\\f3ee",
        "fa-sitemap" => "\\f0e8",
        "fa-sith" => "\\f512",
        "fa-skating" => "\\f7c5",
        "fa-skeleton" => "\\f620",
        "fa-sketch" => "\\f7c6",
        "fa-ski-jump" => "\\f7c7",
        "fa-ski-lift" => "\\f7c8",
        "fa-skiing" => "\\f7c9",
        "fa-skiing-nordic" => "\\f7ca",
        "fa-skull" => "\\f54c",
        "fa-skull-cow" => "\\f8de",
        "fa-skull-crossbones" => "\\f714",
        "fa-skyatlas" => "\\f216",
        "fa-skype" => "\\f17e",
        "fa-slack" => "\\f198",
        "fa-slack-hash" => "\\f3ef",
        "fa-slash" => "\\f715",
        "fa-sledding" => "\\f7cb",
        "fa-sleigh" => "\\f7cc",
        "fa-sliders-h" => "\\f1de",
        "fa-sliders-h-square" => "\\f3f0",
        "fa-sliders-v" => "\\f3f1",
        "fa-sliders-v-square" => "\\f3f2",
        "fa-slideshare" => "\\f1e7",
        "fa-smile" => "\\f118",
        "fa-smile-beam" => "\\f5b8",
        "fa-smile-plus" => "\\f5b9",
        "fa-smile-wink" => "\\f4da",
        "fa-smog" => "\\f75f",
        "fa-smoke" => "\\f760",
        "fa-smoking" => "\\f48d",
        "fa-smoking-ban" => "\\f54d",
        "fa-sms" => "\\f7cd",
        "fa-snake" => "\\f716",
        "fa-snapchat" => "\\f2ab",
        "fa-snapchat-ghost" => "\\f2ac",
        "fa-snapchat-square" => "\\f2ad",
        "fa-snooze" => "\\f880",
        "fa-snow-blowing" => "\\f761",
        "fa-snowboarding" => "\\f7ce",
        "fa-snowflake" => "\\f2dc",
        "fa-snowflakes" => "\\f7cf",
        "fa-snowman" => "\\f7d0",
        "fa-snowmobile" => "\\f7d1",
        "fa-snowplow" => "\\f7d2",
        "fa-socks" => "\\f696",
        "fa-solar-panel" => "\\f5ba",
        "fa-sort" => "\\f0dc",
        "fa-sort-alpha-down" => "\\f15d",
        "fa-sort-alpha-down-alt" => "\\f881",
        "fa-sort-alpha-up" => "\\f15e",
        "fa-sort-alpha-up-alt" => "\\f882",
        "fa-sort-alt" => "\\f883",
        "fa-sort-amount-down" => "\\f160",
        "fa-sort-amount-down-alt" => "\\f884",
        "fa-sort-amount-up" => "\\f161",
        "fa-sort-amount-up-alt" => "\\f885",
        "fa-sort-down" => "\\f0dd",
        "fa-sort-numeric-down" => "\\f162",
        "fa-sort-numeric-down-alt" => "\\f886",
        "fa-sort-numeric-up" => "\\f163",
        "fa-sort-numeric-up-alt" => "\\f887",
        "fa-sort-shapes-down" => "\\f888",
        "fa-sort-shapes-down-alt" => "\\f889",
        "fa-sort-shapes-up" => "\\f88a",
        "fa-sort-shapes-up-alt" => "\\f88b",
        "fa-sort-size-down" => "\\f88c",
        "fa-sort-size-down-alt" => "\\f88d",
        "fa-sort-size-up" => "\\f88e",
        "fa-sort-size-up-alt" => "\\f88f",
        "fa-sort-up" => "\\f0de",
        "fa-soundcloud" => "\\f1be",
        "fa-soup" => "\\f823",
        "fa-sourcetree" => "\\f7d3",
        "fa-spa" => "\\f5bb",
        "fa-space-shuttle" => "\\f197",
        "fa-spade" => "\\f2f4",
        "fa-sparkles" => "\\f890",
        "fa-speakap" => "\\f3f3",
        "fa-speaker" => "\\f8df",
        "fa-speaker-deck" => "\\f83c",
        "fa-speakers" => "\\f8e0",
        "fa-spell-check" => "\\f891",
        "fa-spider" => "\\f717",
        "fa-spider-black-widow" => "\\f718",
        "fa-spider-web" => "\\f719",
        "fa-spinner" => "\\f110",
        "fa-spinner-third" => "\\f3f4",
        "fa-splotch" => "\\f5bc",
        "fa-spotify" => "\\f1bc",
        "fa-spray-can" => "\\f5bd",
        "fa-square" => "\\f0c8",
        "fa-square-full" => "\\f45c",
        "fa-square-root" => "\\f697",
        "fa-square-root-alt" => "\\f698",
        "fa-squarespace" => "\\f5be",
        "fa-squirrel" => "\\f71a",
        "fa-stack-exchange" => "\\f18d",
        "fa-stack-overflow" => "\\f16c",
        "fa-stackpath" => "\\f842",
        "fa-staff" => "\\f71b",
        "fa-stamp" => "\\f5bf",
        "fa-star" => "\\f005",
        "fa-star-and-crescent" => "\\f699",
        "fa-star-christmas" => "\\f7d4",
        "fa-star-exclamation" => "\\f2f3",
        "fa-star-half" => "\\f089",
        "fa-star-half-alt" => "\\f5c0",
        "fa-star-of-david" => "\\f69a",
        "fa-star-of-life" => "\\f621",
        "fa-stars" => "\\f762",
        "fa-staylinked" => "\\f3f5",
        "fa-steak" => "\\f824",
        "fa-steam" => "\\f1b6",
        "fa-steam-square" => "\\f1b7",
        "fa-steam-symbol" => "\\f3f6",
        "fa-steering-wheel" => "\\f622",
        "fa-step-backward" => "\\f048",
        "fa-step-forward" => "\\f051",
        "fa-stethoscope" => "\\f0f1",
        "fa-sticker-mule" => "\\f3f7",
        "fa-sticky-note" => "\\f249",
        "fa-stocking" => "\\f7d5",
        "fa-stomach" => "\\f623",
        "fa-stop" => "\\f04d",
        "fa-stop-circle" => "\\f28d",
        "fa-stopwatch" => "\\f2f2",
        "fa-store" => "\\f54e",
        "fa-store-alt" => "\\f54f",
        "fa-strava" => "\\f428",
        "fa-stream" => "\\f550",
        "fa-street-view" => "\\f21d",
        "fa-stretcher" => "\\f825",
        "fa-strikethrough" => "\\f0cc",
        "fa-stripe" => "\\f429",
        "fa-stripe-s" => "\\f42a",
        "fa-stroopwafel" => "\\f551",
        "fa-studiovinari" => "\\f3f8",
        "fa-stumbleupon" => "\\f1a4",
        "fa-stumbleupon-circle" => "\\f1a3",
        "fa-subscript" => "\\f12c",
        "fa-subway" => "\\f239",
        "fa-suitcase" => "\\f0f2",
        "fa-suitcase-rolling" => "\\f5c1",
        "fa-sun" => "\\f185",
        "fa-sun-cloud" => "\\f763",
        "fa-sun-dust" => "\\f764",
        "fa-sun-haze" => "\\f765",
        "fa-sunglasses" => "\\f892",
        "fa-sunrise" => "\\f766",
        "fa-sunset" => "\\f767",
        "fa-superpowers" => "\\f2dd",
        "fa-superscript" => "\\f12b",
        "fa-supple" => "\\f3f9",
        "fa-surprise" => "\\f5c2",
        "fa-suse" => "\\f7d6",
        "fa-swatchbook" => "\\f5c3",
        "fa-swift" => "\\f8e1",
        "fa-swimmer" => "\\f5c4",
        "fa-swimming-pool" => "\\f5c5",
        "fa-sword" => "\\f71c",
        "fa-swords" => "\\f71d",
        "fa-symfony" => "\\f83d",
        "fa-synagogue" => "\\f69b",
        "fa-sync" => "\\f021",
        "fa-sync-alt" => "\\f2f1",
        "fa-syringe" => "\\f48e",
        "fa-table" => "\\f0ce",
        "fa-table-tennis" => "\\f45d",
        "fa-tablet" => "\\f10a",
        "fa-tablet-alt" => "\\f3fa",
        "fa-tablet-android" => "\\f3fb",
        "fa-tablet-android-alt" => "\\f3fc",
        "fa-tablet-rugged" => "\\f48f",
        "fa-tablets" => "\\f490",
        "fa-tachometer" => "\\f0e4",
        "fa-tachometer-alt" => "\\f3fd",
        "fa-tachometer-alt-average" => "\\f624",
        "fa-tachometer-alt-fast" => "\\f625",
        "fa-tachometer-alt-fastest" => "\\f626",
        "fa-tachometer-alt-slow" => "\\f627",
        "fa-tachometer-alt-slowest" => "\\f628",
        "fa-tachometer-average" => "\\f629",
        "fa-tachometer-fast" => "\\f62a",
        "fa-tachometer-fastest" => "\\f62b",
        "fa-tachometer-slow" => "\\f62c",
        "fa-tachometer-slowest" => "\\f62d",
        "fa-taco" => "\\f826",
        "fa-tag" => "\\f02b",
        "fa-tags" => "\\f02c",
        "fa-tally" => "\\f69c",
        "fa-tanakh" => "\\f827",
        "fa-tape" => "\\f4db",
        "fa-tasks" => "\\f0ae",
        "fa-tasks-alt" => "\\f828",
        "fa-taxi" => "\\f1ba",
        "fa-teamspeak" => "\\f4f9",
        "fa-teeth" => "\\f62e",
        "fa-teeth-open" => "\\f62f",
        "fa-telegram" => "\\f2c6",
        "fa-telegram-plane" => "\\f3fe",
        "fa-temperature-frigid" => "\\f768",
        "fa-temperature-high" => "\\f769",
        "fa-temperature-hot" => "\\f76a",
        "fa-temperature-low" => "\\f76b",
        "fa-tencent-weibo" => "\\f1d5",
        "fa-tenge" => "\\f7d7",
        "fa-tennis-ball" => "\\f45e",
        "fa-terminal" => "\\f120",
        "fa-text" => "\\f893",
        "fa-text-height" => "\\f034",
        "fa-text-size" => "\\f894",
        "fa-text-width" => "\\f035",
        "fa-th" => "\\f00a",
        "fa-th-large" => "\\f009",
        "fa-th-list" => "\\f00b",
        "fa-the-red-yeti" => "\\f69d",
        "fa-theater-masks" => "\\f630",
        "fa-themeco" => "\\f5c6",
        "fa-themeisle" => "\\f2b2",
        "fa-thermometer" => "\\f491",
        "fa-thermometer-empty" => "\\f2cb",
        "fa-thermometer-full" => "\\f2c7",
        "fa-thermometer-half" => "\\f2c9",
        "fa-thermometer-quarter" => "\\f2ca",
        "fa-thermometer-three-quarters" => "\\f2c8",
        "fa-theta" => "\\f69e",
        "fa-think-peaks" => "\\f731",
        "fa-thumbs-down" => "\\f165",
        "fa-thumbs-up" => "\\f164",
        "fa-thumbtack" => "\\f08d",
        "fa-thunderstorm" => "\\f76c",
        "fa-thunderstorm-moon" => "\\f76d",
        "fa-thunderstorm-sun" => "\\f76e",
        "fa-ticket" => "\\f145",
        "fa-ticket-alt" => "\\f3ff",
        "fa-tilde" => "\\f69f",
        "fa-times" => "\\f00d",
        "fa-times-circle" => "\\f057",
        "fa-times-hexagon" => "\\f2ee",
        "fa-times-octagon" => "\\f2f0",
        "fa-times-square" => "\\f2d3",
        "fa-tint" => "\\f043",
        "fa-tint-slash" => "\\f5c7",
        "fa-tire" => "\\f631",
        "fa-tire-flat" => "\\f632",
        "fa-tire-pressure-warning" => "\\f633",
        "fa-tire-rugged" => "\\f634",
        "fa-tired" => "\\f5c8",
        "fa-toggle-off" => "\\f204",
        "fa-toggle-on" => "\\f205",
        "fa-toilet" => "\\f7d8",
        "fa-toilet-paper" => "\\f71e",
        "fa-toilet-paper-alt" => "\\f71f",
        "fa-tombstone" => "\\f720",
        "fa-tombstone-alt" => "\\f721",
        "fa-toolbox" => "\\f552",
        "fa-tools" => "\\f7d9",
        "fa-tooth" => "\\f5c9",
        "fa-toothbrush" => "\\f635",
        "fa-torah" => "\\f6a0",
        "fa-torii-gate" => "\\f6a1",
        "fa-tornado" => "\\f76f",
        "fa-tractor" => "\\f722",
        "fa-trade-federation" => "\\f513",
        "fa-trademark" => "\\f25c",
        "fa-traffic-cone" => "\\f636",
        "fa-traffic-light" => "\\f637",
        "fa-traffic-light-go" => "\\f638",
        "fa-traffic-light-slow" => "\\f639",
        "fa-traffic-light-stop" => "\\f63a",
        "fa-train" => "\\f238",
        "fa-tram" => "\\f7da",
        "fa-transgender" => "\\f224",
        "fa-transgender-alt" => "\\f225",
        "fa-trash" => "\\f1f8",
        "fa-trash-alt" => "\\f2ed",
        "fa-trash-restore" => "\\f829",
        "fa-trash-restore-alt" => "\\f82a",
        "fa-trash-undo" => "\\f895",
        "fa-trash-undo-alt" => "\\f896",
        "fa-treasure-chest" => "\\f723",
        "fa-tree" => "\\f1bb",
        "fa-tree-alt" => "\\f400",
        "fa-tree-christmas" => "\\f7db",
        "fa-tree-decorated" => "\\f7dc",
        "fa-tree-large" => "\\f7dd",
        "fa-tree-palm" => "\\f82b",
        "fa-trees" => "\\f724",
        "fa-trello" => "\\f181",
        "fa-triangle" => "\\f2ec",
        "fa-triangle-music" => "\\f8e2",
        "fa-tripadvisor" => "\\f262",
        "fa-trophy" => "\\f091",
        "fa-trophy-alt" => "\\f2eb",
        "fa-truck" => "\\f0d1",
        "fa-truck-container" => "\\f4dc",
        "fa-truck-couch" => "\\f4dd",
        "fa-truck-loading" => "\\f4de",
        "fa-truck-monster" => "\\f63b",
        "fa-truck-moving" => "\\f4df",
        "fa-truck-pickup" => "\\f63c",
        "fa-truck-plow" => "\\f7de",
        "fa-truck-ramp" => "\\f4e0",
        "fa-trumpet" => "\\f8e3",
        "fa-tshirt" => "\\f553",
        "fa-tty" => "\\f1e4",
        "fa-tumblr" => "\\f173",
        "fa-tumblr-square" => "\\f174",
        "fa-turkey" => "\\f725",
        "fa-turntable" => "\\f8e4",
        "fa-turtle" => "\\f726",
        "fa-tv" => "\\f26c",
        "fa-tv-alt" => "\\f8e5",
        "fa-tv-music" => "\\f8e6",
        "fa-tv-retro" => "\\f401",
        "fa-twitch" => "\\f1e8",
        "fa-twitter" => "\\f099",
        "fa-twitter-square" => "\\f081",
        "fa-typewriter" => "\\f8e7",
        "fa-typo3" => "\\f42b",
        "fa-uber" => "\\f402",
        "fa-ubuntu" => "\\f7df",
        "fa-uikit" => "\\f403",
        "fa-umbraco" => "\\f8e8",
        "fa-umbrella" => "\\f0e9",
        "fa-umbrella-beach" => "\\f5ca",
        "fa-underline" => "\\f0cd",
        "fa-undo" => "\\f0e2",
        "fa-undo-alt" => "\\f2ea",
        "fa-unicorn" => "\\f727",
        "fa-union" => "\\f6a2",
        "fa-uniregistry" => "\\f404",
        "fa-universal-access" => "\\f29a",
        "fa-university" => "\\f19c",
        "fa-unlink" => "\\f127",
        "fa-unlock" => "\\f09c",
        "fa-unlock-alt" => "\\f13e",
        "fa-untappd" => "\\f405",
        "fa-upload" => "\\f093",
        "fa-ups" => "\\f7e0",
        "fa-usb" => "\\f287",
        "fa-usb-drive" => "\\f8e9",
        "fa-usd-circle" => "\\f2e8",
        "fa-usd-square" => "\\f2e9",
        "fa-user" => "\\f007",
        "fa-user-alt" => "\\f406",
        "fa-user-alt-slash" => "\\f4fa",
        "fa-user-astronaut" => "\\f4fb",
        "fa-user-chart" => "\\f6a3",
        "fa-user-check" => "\\f4fc",
        "fa-user-circle" => "\\f2bd",
        "fa-user-clock" => "\\f4fd",
        "fa-user-cog" => "\\f4fe",
        "fa-user-cowboy" => "\\f8ea",
        "fa-user-crown" => "\\f6a4",
        "fa-user-edit" => "\\f4ff",
        "fa-user-friends" => "\\f500",
        "fa-user-graduate" => "\\f501",
        "fa-user-hard-hat" => "\\f82c",
        "fa-user-headset" => "\\f82d",
        "fa-user-injured" => "\\f728",
        "fa-user-lock" => "\\f502",
        "fa-user-md" => "\\f0f0",
        "fa-user-md-chat" => "\\f82e",
        "fa-user-minus" => "\\f503",
        "fa-user-music" => "\\f8eb",
        "fa-user-ninja" => "\\f504",
        "fa-user-nurse" => "\\f82f",
        "fa-user-plus" => "\\f234",
        "fa-user-secret" => "\\f21b",
        "fa-user-shield" => "\\f505",
        "fa-user-slash" => "\\f506",
        "fa-user-tag" => "\\f507",
        "fa-user-tie" => "\\f508",
        "fa-user-times" => "\\f235",
        "fa-users" => "\\f0c0",
        "fa-users-class" => "\\f63d",
        "fa-users-cog" => "\\f509",
        "fa-users-crown" => "\\f6a5",
        "fa-users-medical" => "\\f830",
        "fa-usps" => "\\f7e1",
        "fa-ussunnah" => "\\f407",
        "fa-utensil-fork" => "\\f2e3",
        "fa-utensil-knife" => "\\f2e4",
        "fa-utensil-spoon" => "\\f2e5",
        "fa-utensils" => "\\f2e7",
        "fa-utensils-alt" => "\\f2e6",
        "fa-vaadin" => "\\f408",
        "fa-value-absolute" => "\\f6a6",
        "fa-vector-square" => "\\f5cb",
        "fa-venus" => "\\f221",
        "fa-venus-double" => "\\f226",
        "fa-venus-mars" => "\\f228",
        "fa-vhs" => "\\f8ec",
        "fa-viacoin" => "\\f237",
        "fa-viadeo" => "\\f2a9",
        "fa-viadeo-square" => "\\f2aa",
        "fa-vial" => "\\f492",
        "fa-vials" => "\\f493",
        "fa-viber" => "\\f409",
        "fa-video" => "\\f03d",
        "fa-video-plus" => "\\f4e1",
        "fa-video-slash" => "\\f4e2",
        "fa-vihara" => "\\f6a7",
        "fa-vimeo" => "\\f40a",
        "fa-vimeo-square" => "\\f194",
        "fa-vimeo-v" => "\\f27d",
        "fa-vine" => "\\f1ca",
        "fa-violin" => "\\f8ed",
        "fa-vk" => "\\f189",
        "fa-vnv" => "\\f40b",
        "fa-voicemail" => "\\f897",
        "fa-volcano" => "\\f770",
        "fa-volleyball-ball" => "\\f45f",
        "fa-volume" => "\\f6a8",
        "fa-volume-down" => "\\f027",
        "fa-volume-mute" => "\\f6a9",
        "fa-volume-off" => "\\f026",
        "fa-volume-slash" => "\\f2e2",
        "fa-volume-up" => "\\f028",
        "fa-vote-nay" => "\\f771",
        "fa-vote-yea" => "\\f772",
        "fa-vr-cardboard" => "\\f729",
        "fa-vuejs" => "\\f41f",
        "fa-wagon-covered" => "\\f8ee",
        "fa-walker" => "\\f831",
        "fa-walkie-talkie" => "\\f8ef",
        "fa-walking" => "\\f554",
        "fa-wallet" => "\\f555",
        "fa-wand" => "\\f72a",
        "fa-wand-magic" => "\\f72b",
        "fa-warehouse" => "\\f494",
        "fa-warehouse-alt" => "\\f495",
        "fa-washer" => "\\f898",
        "fa-watch" => "\\f2e1",
        "fa-watch-calculator" => "\\f8f0",
        "fa-watch-fitness" => "\\f63e",
        "fa-water" => "\\f773",
        "fa-water-lower" => "\\f774",
        "fa-water-rise" => "\\f775",
        "fa-wave-sine" => "\\f899",
        "fa-wave-square" => "\\f83e",
        "fa-wave-triangle" => "\\f89a",
        "fa-waveform" => "\\f8f1",
        "fa-waveform-path" => "\\f8f2",
        "fa-waze" => "\\f83f",
        "fa-webcam" => "\\f832",
        "fa-webcam-slash" => "\\f833",
        "fa-weebly" => "\\f5cc",
        "fa-weibo" => "\\f18a",
        "fa-weight" => "\\f496",
        "fa-weight-hanging" => "\\f5cd",
        "fa-weixin" => "\\f1d7",
        "fa-whale" => "\\f72c",
        "fa-whatsapp" => "\\f232",
        "fa-whatsapp-square" => "\\f40c",
        "fa-wheat" => "\\f72d",
        "fa-wheelchair" => "\\f193",
        "fa-whistle" => "\\f460",
        "fa-whmcs" => "\\f40d",
        "fa-wifi" => "\\f1eb",
        "fa-wifi-1" => "\\f6aa",
        "fa-wifi-2" => "\\f6ab",
        "fa-wifi-slash" => "\\f6ac",
        "fa-wikipedia-w" => "\\f266",
        "fa-wind" => "\\f72e",
        "fa-wind-turbine" => "\\f89b",
        "fa-wind-warning" => "\\f776",
        "fa-window" => "\\f40e",
        "fa-window-alt" => "\\f40f",
        "fa-window-close" => "\\f410",
        "fa-window-maximize" => "\\f2d0",
        "fa-window-minimize" => "\\f2d1",
        "fa-window-restore" => "\\f2d2",
        "fa-windows" => "\\f17a",
        "fa-windsock" => "\\f777",
        "fa-wine-bottle" => "\\f72f",
        "fa-wine-glass" => "\\f4e3",
        "fa-wine-glass-alt" => "\\f5ce",
        "fa-wix" => "\\f5cf",
        "fa-wizards-of-the-coast" => "\\f730",
        "fa-wolf-pack-battalion" => "\\f514",
        "fa-won-sign" => "\\f159",
        "fa-wordpress" => "\\f19a",
        "fa-wordpress-simple" => "\\f411",
        "fa-wpbeginner" => "\\f297",
        "fa-wpexplorer" => "\\f2de",
        "fa-wpforms" => "\\f298",
        "fa-wpressr" => "\\f3e4",
        "fa-wreath" => "\\f7e2",
        "fa-wrench" => "\\f0ad",
        "fa-x-ray" => "\\f497",
        "fa-xbox" => "\\f412",
        "fa-xing" => "\\f168",
        "fa-xing-square" => "\\f169",
        "fa-y-combinator" => "\\f23b",
        "fa-yahoo" => "\\f19e",
        "fa-yammer" => "\\f840",
        "fa-yandex" => "\\f413",
        "fa-yandex-international" => "\\f414",
        "fa-yarn" => "\\f7e3",
        "fa-yelp" => "\\f1e9",
        "fa-yen-sign" => "\\f157",
        "fa-yin-yang" => "\\f6ad",
        "fa-yoast" => "\\f2b1",
        "fa-youtube" => "\\f167",
        "fa-youtube-square" => "\\f431",
        "fa-zhihu" => "\\f63f",
        "nx-add" => "",
        "nx-add-fixed" => "",
        "nx-alert" => "",
        "nx-apps" => "",
        "nx-archive" => "",
        "nx-archived" => "",
        "nx-arrow-down" => "",
        "nx-arrow-right" => "",
        "nx-arrow-up" => "",
        "nx-assignee" => "",
        "nx-at" => "",
        "nx-attachment" => "",
        "nx-bold-arrow-down" => "",
        "nx-bold-arrow-left" => "",
        "nx-bold-arrow-right" => "",
        "nx-bold-arrow-up" => "",
        "nx-box-arrow-right" => "",
        "nx-calendar" => "",
        "nx-cards" => "",
        "nx-caret-down" => "",
        "nx-caret-left" => "",
        "nx-caret-right" => "",
        "nx-caret-up" => "",
        "nx-chart" => "",
        "nx-check" => "",
        "nx-checklist" => "",
        "nx-edit-square" => "",
        "nx-close" => "",
        "nx-closed-eye" => "",
        "nx-cog" => "",
        "nx-columns" => "",
        "nx-comment" => "",
        "nx-connected" => "",
        "nx-contract" => "",
        "nx-currency" => "",
        "nx-database" => "",
        "nx-description" => "",
        "nx-document" => "",
        "nx-dots" => "",
        "nx-download" => "",
        "nx-edit" => "",
        "nx-email" => "",
        "nx-expand" => "",
        "nx-export" => "",
        "nx-fab" => "",
        "nx-field-id" => "",
        "nx-folder" => "",
        "nx-formula" => "",
        "nx-github" => "",
        "nx-help" => "",
        "nx-home" => "",
        "nx-improvements" => "",
        "nx-info" => "",
        "nx-intercom" => "",
        "nx-justify-left" => "",
        "nx-kanban" => "",
        "nx-label" => "",
        "nx-landscape" => "",
        "nx-leave" => "",
        "nx-left" => "",
        "nx-arrow-left" => "",
        "nx-letter" => "",
        "nx-loading" => "",
        "nx-lock" => "󩈊",
        "nx-logout" => "",
        "nx-metrics" => "",
        "nx-moved" => "",
        "nx-my-desk" => "",
        "nx-new-filter" => "",
        "nx-notifications" => "",
        "nx-number" => "",
        "nx-ocr" => "",
        "nx-open-eye" => "",
        "nx-order" => "",
        "nx-phase" => "",
        "nx-phone" => "",
        "nx-pipes" => "",
        "nx-play" => "",
        "nx-plus" => "󩉪",
        "nx-radio" => "",
        "nx-refresh" => "",
        "nx-reports" => "",
        "nx-search" => "",
        "nx-select" => "",
        "nx-share" => "",
        "nx-sprint" => "",
        "nx-star" => "",
        "nx-star-fill" => "",
        "nx-startform" => "󩉪",
        "nx-stopwatch" => "",
        "nx-time" => "",
        "nx-timeline" => "",
        "nx-timephase" => "",
        "nx-trash" => "",
        "nx-upload" => "",
        "nx-keyboard" => "",
        "nx-events" => "",
        "nx-longtext" => "",
        "nx-datetime" => "",
        "nx-help-rounded" => "",
        "nx-empty-document" => "",
        "nx-hourglass" => "",
        "nx-robot-head" => "",
        "nx-tool" => "",
        "nx-globe" => "",
        "nx-play-filled" => "",
        "nx-pen" => "",
        "nx-lamp" => "",
        "nx-unlink" => "",
        "nx-sort" => "",
        "nx-help-filled" => "",
        "nx-start-form" => "",
        "nx-field" => "",
        "nx-write-email" => "",
        "nx-duplicate" => "",
        "nx-emo" => "",
    );
    private $_icons = array();
    private $_current_style;
    private $_current_icon;
    private $_custom_class;
    private $_render = false;
    private $_spin = false;

    public function __construct()
    {
        $env = new Env();
        if ($env->get("APP_ENV") === "local") {
            $this->_render = true;
        }
    }

    public function spin(): StaticFilesIcons{
        $this->_spin = true;
        return $this;
    }

    public function icon($icon): StaticFilesIcons
    {

        $this->_current_style = null;
        $this->_custom_class = null;

        $this->_current_icon = $icon;
        array_push($this->_icons, $icon);
        return $this;
    }

    public function css($class): StaticFilesIcons
    {
        $this->_custom_class = $class;
        return $this;
    }

    public function style($style): StaticFilesIcons
    {
        $this->_current_style = $style;
        if (!array_key_exists($style, $this->_styles)) {
            $this->_styles[$style] = true;
        }
        return $this;
    }

    public function output($custom_html_tag = "i"): string
    {
        if($this->_spin) $this->_custom_class .= " fa-spin nx-fs-22";
        return "<" . $custom_html_tag . " class=\"" . $this->_custom_class . " " . $this->_current_style . " " . $this->_current_icon . "\"></" . $custom_html_tag . ">";
    }

    private function unique()
    {
        $this->_icons = array_unique($this->_icons);
    }

    private function include($style = "far"): bool
    {
        return array_key_exists($style, $this->_styles);
    }

    private function document_init(): string
    {
        $document = PHP_EOL . "/*$this->_technology_name-BEGIN*/";
        if ($this->include("fab")) $document .= PHP_EOL . "@font-face{font-family:\"Font Awesome 5 Brands\";font-style:normal;font-weight:normal;font-display:swap;src:url(../fonts/fontawesome/fa-brands-400.eot);src:url(../fonts/fontawesome/fa-brands-400.eot?#iefix) format(\"embedded-opentype\"),url(../fonts/fontawesome/fa-brands-400.woff2) format(\"woff2\"),url(../fonts/fontawesome/fa-brands-400.woff) format(\"woff\"),url(../fonts/fontawesome/fa-brands-400.ttf) format(\"truetype\"),url(../fonts/fontawesome/fa-brands-400.svg#fontawesome) format(\"svg\")}.fab{font-family:\"Font Awesome 5 Brands\", serif}";
        if ($this->include("far")) $document .= PHP_EOL . "@font-face{font-family:\"Font Awesome 5 Pro\";font-style:normal;font-weight:400;font-display:swap;src:url(../fonts/fontawesome/fa-regular-400.eot);src:url(../fonts/fontawesome/fa-regular-400.eot?#iefix) format(\"embedded-opentype\"),url(../fonts/fontawesome/fa-regular-400.woff2) format(\"woff2\"),url(../fonts/fontawesome/fa-regular-400.woff) format(\"woff\"),url(../fonts/fontawesome/fa-regular-400.ttf) format(\"truetype\"),url(../fonts/fontawesome/fa-regular-400.svg#fontawesome) format(\"svg\")}.far{font-family:\"Font Awesome 5 Pro\", serif;font-weight:400}";
        if ($this->include("fal")) $document .= PHP_EOL . "@font-face{font-family:\"Font Awesome 5 Pro\";font-style:normal;font-weight:300;font-display:swap;src:url(../fonts/fontawesome/fa-light-300.eot);src:url(../fonts/fontawesome/fa-light-300.eot?#iefix) format(\"embedded-opentype\"),url(../fonts/fontawesome/fa-light-300.woff2) format(\"woff2\"),url(../fonts/fontawesome/fa-light-300.woff) format(\"woff\"),url(../fonts/fontawesome/fa-light-300.ttf) format(\"truetype\"),url(../fonts/fontawesome/fa-light-300.svg#fontawesome) format(\"svg\")}.fal{font-family:\"Font Awesome 5 Pro\", serif;font-weight:300}";
        if ($this->include("fas")) $document .= PHP_EOL . "@font-face{font-family:\"Font Awesome 5 Pro\";font-style:normal;font-weight:900;font-display:swap;src:url(../fonts/fontawesome/fa-solid-900.eot);src:url(../fonts/fontawesome/fa-solid-900.eot?#iefix) format(\"embedded-opentype\"),url(../fonts/fontawesome/fa-solid-900.woff2) format(\"woff2\"),url(../fonts/fontawesome/fa-solid-900.woff) format(\"woff\"),url(../fonts/fontawesome/fa-solid-900.ttf) format(\"truetype\"),url(../fonts/fontawesome/fa-solid-900.svg#fontawesome) format(\"svg\")}.fas{font-family:\"Font Awesome 5 Pro\", serif;font-weight:900}";
        if ($this->include("nex")) $document .= PHP_EOL . "@font-face{font-family:\"nexicons\"; font-style:normal; src: url(../fonts/nexicons/nexicon.woff2) format(\"woff2\"), url(../fonts/nexicons/nexicon.woff) format(\"woff\"), url(../fonts/nexicons/nexicon.ttf) format(\"ttf\"), url(../fonts/nexicons/nexicon.svg#nexicons) format(\"svg\") }  .nex,[class^=nx-]:before { font-family: nexicons, serif; -moz-osx-font-smoothing: grayscale; -webkit-font-smoothing: antialiased; font-style: normal; font-variant: normal; font-weight: 400; speak: none; text-transform: none; vertical-align: middle }";
        $document .= PHP_EOL . ".fa,.fab,.fad,.fal,.far,.fas{-moz-osx-font-smoothing:grayscale;-webkit-font-smoothing:antialiased;display:inline-block;font-style:normal;font-variant:normal;text-rendering:auto;line-height:1}";
        $document .= PHP_EOL . "/*$this->_technology_name-END*/";
        $fonts_css = $this->_path . $this->_fonts_files;
        $actual_content = "";
        if (file_exists($fonts_css)) {
            $actual_content = file_get_contents($fonts_css);
            $actual_content = preg_replace("/\/\*$this->_technology_name-BEGIN[\s\S]+?\/\*$this->_technology_name-END\*\//", '', $actual_content);
            $actual_content = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", PHP_EOL, $actual_content);
        }
        $file_content = $actual_content;
        $file_content .= $document;
        $file = fopen($this->_path . $this->_fonts_files, "w") or die("Unable to open file!");
        fwrite($file, $file_content);
        fclose($file);
        return $document;
    }

    public function __destruct()
    {
        $text = new Text();
        if ($this->_render) {
            $this->unique();
            $icons = $this->_icons;
            $main = $this->_main;

            $this->document_init();

            $file_content = "";
            if (file_exists($this->_path . $this->_filename)) {
                $file_content = file_get_contents($this->_path . $this->_filename);
            }

            foreach ($icons as $key => $value) {
                if (array_key_exists($value, $main)) {
                    $icon_string = "." . $value . ":before{";
                    $icon_string .= "content: \"" . $main[$value] . "\";";
                    if (strpos($value, "nx") !== false) {
                        $icon_string .= "font-weight:400;";
                    }
                    $icon_string .= "}";

                    if (strpos($file_content, $icon_string) === false) {
                        $file_content .= $icon_string;
                    }
                }
            }
            $file = fopen($this->_path . $this->_filename, "w") or die("Unable to open file!");
            fwrite($file, $file_content);
            fclose($file);
        }
    }


}